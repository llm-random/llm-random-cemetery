...
NON_ACCURACY_METRICS = {"perplexity", "word_perplexity", "byte_perplexity", "bits_per_byte"}
...
def sort_key(row: Row):
    method_rank = (
        METHOD_ORDER.index(row.method) if row.method in METHOD_ORDER else len(METHOD_ORDER)
    )
...
def render(rows: list[Row], handle, precision: int) -> None:
    ...
    header = ["model", "method", "ratio"] + tasks + [
        "avg",
        "num_fewshot",
        "state",
        "run_id",
...
def main():
    ...
    parser.add_argument(
        "--after", default=None, help="only runs created on/after this date (YYYY-MM-DD)"
    )
