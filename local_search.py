...
class LocalSearch:
    ...
    def wait_until_true(self, trange, func, pids, desc):
        all = len(pids)
        if isinstance(desc, str): desc = lambda: desc
    ...
    def wait_for_runs_to_finish_and_get_best(self, pids, param):
        ...
        get_results = lambda: [run.get_results() for run in pids]
        get_results_str = lambda: ", ".join([f"{run.val}: {res}" for run, res in zip(pids, get_results())])
