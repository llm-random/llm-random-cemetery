...
class TrainRun:
    def __init__(self, search, pid, val):
        self.pid = pid
    ...
    def is_queued(self):
        out = run_command(f"ssh -qt {self.server} 'squeue --start --states=R -o \"%.50j\" ; squeue --states=R -o \"%.50j\"' | grep {self.pid} 2>/dev/null")
        return len(out) > 1
...
class LocalSearch:
    ...
    def run_config_server(self, config_path):
        os.system(f"bash scripts/run_exp_remotely.sh {self.server_name} {config_path}")
    ...
    def run_config_dict(self, config, param, val, iter):
        ...
        self.run_config_server(config_path)
        return TrainRun(self, name, val)
    ...
    def wait_for_runs_to_finish_and_get_best(self, pids, param):
        range = trange(4*len(pids), desc="Starting..", leave=True)
        self.wait_until_true(range, TrainRun.is_queued, list(pids), "Waiting for exps to queue")
