...
def get_model(
    ...
    if projected_checkpoint is not None:
        if not projection_init_type:
            ...
            projection = torch.zeros(int(projected_dmodel/4), int(projected_dmodel/4))
            mask = torch.eye(int(projected_dmodel/4)).bool()
            projection = projection.masked_fill(mask, 0.5) #dev change half_2
