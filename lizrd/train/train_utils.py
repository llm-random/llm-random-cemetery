...
def get_model(
    ...
    if (projected_checkpoint is not None) and (projection_init_type is not None):
        ...
        initialize_pruned(pruning_method=pruning, model=model["model"], projected_weights=projected_checkpoint["model"], dmodel=dm, dff=dff, projected_dmodel=projected_dmodel, projected_dff=projected_dff, n_att_heads=n_att_heads, local_rank = local_rank)
