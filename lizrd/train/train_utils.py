...
def get_model(
    ...
    if gradient_checkpointing:
        check_fn = lambda x: isinstance(x, (TransformerBlock, EmbeddingLayer, PredictionHead))
        non_reentrant_wrapper = partial(
            checkpoint_wrapper,
            offload_to_cpu=False,
            checkpoint_impl=CheckpointImpl.NO_REENTRANT,
        )
        apply_activation_checkpointing(model, check_fn=check_fn, checkpoint_wrapper_fn=non_reentrant_wrapper)
