...
def get_model(
    ...
    if projected_checkpoint is not None:
        if not projection_init_type:
            ...
            print(f"get_var_head_projection {mask_1d2}") #dev
            # projection = projection[:, torch.concat([torch.Tensor([True, False])]*n_att_heads*8*4).type(torch.bool)]
            projection = projection[:, torch.concat([mask_1d2]*n_att_heads*8*4)]
        # elif projection_init_type == "head_half":
        #     print("Projection initialization: head_half")
        #     assert (projected_dmodel/n_att_heads)%2 == 0
