...
def job_out_of_time_checkpoint(job_id, is_logging_process, model: Union[torch.nn.Module, FSDP], optimizer, scaler, path: str, rank: int, step: int, batch_size: int, cutoff, joint_loggers: Optional[JointLogger] = None): #TODO params
    """saves the checkpoint"""
    model_path = save_checkpoint(model, optimizer, scaler, path, rank, step, batch_size, cutoff, joint_loggers.loggers)
    timestamp_now = __get_manager_timestamp()
    if is_logging_process:
        with Locker(EXPERIMENT_CHECKPOINT_MANAGER, "r+") as f:
            manager = json.load(f)
...
def end_training_checkpoint(job_id, is_logging_process, model: Union[torch.nn.Module, FSDP], optimizer, scaler, path: str, rank: int, step: int, batch_size: int, cutoff, joint_loggers: Optional[JointLogger] = None):
...
def create_slide_checkpoint(job_id, is_logging_process, model: Union[torch.nn.Module, FSDP], optimizer, scaler, path: str, rank: int, step: int, batch_size: int, cutoff, joint_loggers: Optional[JointLogger] = None):
    """saves checkpoint and creates a manager checkpoint continuation"""
    model_path = save_checkpoint(model, optimizer, scaler, path, rank, step, batch_size, cutoff, joint_loggers.loggers, lr_scheduler_metadata="trapezoidal_slide")
    timestamp_now = __get_manager_timestamp()
    if is_logging_process:
        with Locker(EXPERIMENT_CHECKPOINT_MANAGER, "r+") as f:
            manager = json.load(f)
