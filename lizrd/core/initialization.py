...
def get_init_weight(shape, fan_in, init_type, scale, dtype=torch.bfloat16):
...
def init_kaiming_uniform(shape, fan_in, scale, dtype=torch.bfloat16):
    range_ = scale * (3 / fan_in) ** 0.5
    return torch.zeros(shape, dtype=dtype).uniform_(-range_, range_)


def init_truncated_normal(shape, fan_in, scale, dtype=torch.bfloat16):
...
def get_init_bias(shape, fan_in=None, fan_out=None, dtype=torch.bfloat16):
