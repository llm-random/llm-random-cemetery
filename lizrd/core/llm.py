...
class RMSNorm(nn.Module):
    def __init__(self, dmodel, eps=1e-8):
        ...
        self.ps = torch.full((dmodel,), 2.0)

    def forward(self, x):
        ps = self.ps.to(device=x.device)
        p_norm = torch.mean(torch.pow(torch.abs(x), ps), dim=-1, keepdim=True)
        x = x * torch.pow(p_norm + self.eps, -1 / ps)
