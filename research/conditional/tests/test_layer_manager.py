...
class TestLearningStartAdatempPositive(GeneralTestCase):
    layers = []
    for i in range(10):
        mot = ContinuousMoEAdaTempPositive(dm=8, dff=32, n_experts=4, group_size=2, sparsity_dim=0, temperature=1.0, init_type="kaiming_uniform", init_scale=1.0, expert_size=None,flop_matched=True,share_by_experts=False)
        residual = Residual(
            nn.Sequential(
                OrderedDict(
                    [
        ...
        for name, module in model.named_modules():
            if isinstance(module, ContinuousMoEAdaTempPositive):
                ...
                if step < steps_until_start_temperature_learn:
                    assert (module.temperature_emit.data == 1.0).all()
                else:
                    assert (module.temperature_emit.data != 1.0).all()
        assert checked
