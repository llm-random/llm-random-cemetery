    ...
    def log_heavy(self):
        ...
        for instr_name, instr_time in self.logging_cache.items():
            if "merge_and_process" in instr_name:
                assert isinstance(
                    instr_time, list
                ), f"{instr_name} is not a list, it is {type(instr_time)}"
                assert isinstance(
                    instr_time[0], float
                ), f"{instr_name} is not a list of floats, it is {type(instr_time[0])}. The first element is {instr_time[0]}"
                mean = np.mean(instr_time)
                assert isinstance(
                    mean, float
                ), f"{instr_name} is not a float, it is {type(mean)}"
