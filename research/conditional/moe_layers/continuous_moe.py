...
class ContinuousMoeBaseClass(LoggingLayer):
    ...
    def separate_map_emit(self, x, merge_weights, emit_weights):
        print(f" for {self.n_experts} experts, the shape of x is {x.shape}, and merge are {merge_weights.shape}")
...
class SpeedtestContMoE(nn.Module):
    ...
    def forward(self, x, w, postmerge):
        #merged version
        # x = misc.einsum(
        ...
        if postmerge is None:
            postmerge = misc.einsum("B S c d, B S e c -> B S e d", x, w)
        # x = misc.einsum("B S c d, B S e c -> B S e d", x, w)
        x = misc.einsum("B S e d, e d f -> B S e f", postmerge, self.lin1)
        # x = torch.relu_(x)
        # x = torch.bmm(x, self.lin2)
