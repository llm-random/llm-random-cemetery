...
class ContinuousMoeBaseClass(LoggingLayer):
    ...
    def log_heavy(self):
        ...
        merge_maps = []
        merge_map_names = []
        for instr_name, instr_time in self.logging_cache.items():
            if "merge_and_process" in instr_name:
                assert isinstance(instr_time, list), f"{instr_name} is not a list, it is {type(instr_time)}"
                assert isinstance(instr_time[0], float), f"{instr_name} is not a list of floats, it is {type(instr_time[0])}. The first element is {instr_time[0]}"
                mean = np.mean(instr_time)
                assert isinstance(mean, float), f"{instr_name} is not a float, it is {type(mean)}"
        ...
        default_time = np.mean(self.logging_cache["merge_and_process_B_S_g_d"])
        ...
        log["process_and_emit_time"] = self.logging_cache[
            "process_and_emit_B_S_g_d"
        ]
        log["best_merge_to_emit_ratio"] = (
            merge_best / self.logging_cache["process_and_emit_B_S_g_d"]
        )
        log["worst_merge_to_emit_ratio"] = (
            merge_worst / self.logging_cache["process_and_emit_B_S_g_d"]
        )
