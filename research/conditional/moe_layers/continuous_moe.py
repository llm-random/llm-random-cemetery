...
class MoTRouter(torch.nn.Module):
    ...
    init_type: str
    init_scale: float
    parent_module: Union[torch.nn.Module, None] = None
    ...
    def forward(self, x):
        ...
        self.parent_module.update_cache_for_logging("merge_logits", merge_logits)
        # shape of merge_logits is (free_dimension, agrr_dimension // group_size, group_size, n_experts)
        temp_merge, temp_emit = self.parent_module.get_temperature()
        merge_softmax_dim = -2
        emit_softmax_dim = -1 if self.parent_module.emit_softmax_over_experts else -2
        ...
        self.parent_module.update_cache_for_logging("merge_weights", merge_weights)
        self.parent_module.update_cache_for_logging("emit_weights", emit_weights)
        if self.parent_module.use_discrete_routing:
...
class ContinuousMoeBaseClass(LoggingLayer):
    ...
    def forward(self, x):
        if self.router.parent_module is None:
            self.router.parent_module = self
        x = self.reshape_into_groups(x)
        merge_weights, emit_weights = self.router(x)
