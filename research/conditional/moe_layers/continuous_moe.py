...
class ContinuousMoeBaseClass(LoggingLayer):
    ...
    def reshape_into_token_groups(self, x):
        ...
        if self.sparsity_dim == 0:
            ...
            x = einops.rearrange(x, "B S d -> S B d", g=self.group_size)
    ...
    def get_merge_and_emit_weights(self, x):
        merge_logits = misc.einsum("S g d, d e -> S e g", x, self.controller)
    ...
    def merge_map_emit(self, x, merge_weights, emit_weights):
        ...
        output_order = "S g d"
        # output_order = "g d S"
        # sig = f"{input_order} -> {output_order}"
        # x = einops.rearrange(
        #     x, sig
        # )
        x = misc.einsum(
            f"{output_order}, S e c, d e f -> S e f",
            x,
            merge_weights,
            self.lin1,
    ...
    def reshape_into_original(self, x):
        if self.sparsity_dim == 0:
            out = einops.rearrange(x, "S g d -> g S d")
        elif self.sparsity_dim == 1:
