...
class TokenChoiceFF(LoggingLayer):
    def __init__(
        ...
        self.moe_values_exp = moe_values_exp if moe_values_exp != -1 else torch.nn.Parameter(torch.tensor(1.0))
        self.gate = None

        get_gate = init_gate(self, dmodel, get_router_values_from, init_scale, init_type, n_experts)
    ...
    def merge(
        ...
        if self.moe_values_exp != 1. or not isinstance(self.moe_values_exp, float):
