...
class ExpertMLPKAN(LoggingLayer):
    def __init__(
        ...
        if kan_square:
            self.kan_experts = torch.nn.ModuleList(
                [KanSquareLatent(dmodel=dmodel, dff=kan_bottleneck_size) for _ in range(n_experts)]
            )
        else:
            self.kan_experts = torch.nn.ModuleList(
                [MlpKan_norelu(dmodel=dmodel, dff=kan_bottleneck_size) for _ in range(n_experts)]
            )
    ...
    def calculate_kan_bottlneck(self, kan_square):
        ...
        if kan_square:
            kan_bottlneck = int((math.sqrt(self.dmodel*(20*self.expert_size + self.dmodel)) - self.dmodel)/10)
        else:
