...
class ExpertFF(LoggingLayer):
    ...
    def pre_mocked_gating(self, x):
        if self.use_mock_gating_g > 0:
            mul = einsum("n_experts_1 capacity dmodel, dmodel n_experts_2 -> n_experts_1 capacity n_experts_2", x, self.mock_gating)
...
class ExpertProjectedFF(LoggingLayer):
    def __init__(
        ...
        size = (n_experts, self.weights_base_size) if self.project_whole_matrix else \
            (n_experts, expert_size, self.weights_base_size)
        if self.use_id_init:
            if self.project_whole_matrix and self.weights_base_size == n_experts:
                ...
                raise NotImplementedError("ID initialization is not implemented for this case")
        else:
            self.neuron_selector = self.init_fun(shape=size, fan_in=self.weights_base_size)
    ...
    def calculate_experts(self):
        selector = self.neuron_selector if self.project_skip_softmax else self.selector_act(self.neuron_selector)
        self.iter += 1
        if self.iter < self.detach_projection_for_iters:
            selector = selector.detach()
        if self.project_whole_matrix:
            lin1 = einsum("n_experts weights_base, expert_size weights_base dmodel "
                          "-> n_experts dmodel expert_size",
                          selector, self.lin1_weight)
            lin2 = einsum("n_experts weights_base, expert_size weights_base doutput "
