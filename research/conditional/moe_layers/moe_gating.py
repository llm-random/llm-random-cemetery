...
from research.conditional.moe_layers.load_balancing_loss import (
    calculate_load_balancing_loss, calculate_z_loss
)
...
class TokenGating(MoeGating):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.zloss_weight != 0:
            ...
            return self.apply_capacity(capacity, expert_index, gate_out, n_tokens, zloss_weight=self.zloss_weight, router_logits=router_logits)
        else:
            return self.apply_capacity(capacity, expert_index, gate_out, n_tokens)

    def calculate_balancing_loss(self, gate_out, expert_mask, zloss_weight=0, router_logits=None):
    ...
    def apply_capacity(self, capacity, expert_index, gate_out, n_tokens, zloss_weight=0, router_logits=None):
        ...
        self.calculate_balancing_loss(gate_out, expert_mask, zloss_weight=zloss_weight, router_logits=router_logits)
