...
def init_gate(router, dmodel, get_router_values_from, init_scale, init_type, n_experts):
    if get_router_values_from == "weights":
        init = get_init_fun(init_type=init_type, init_scale=init_scale)
        router.gate = init((dmodel, n_experts), dmodel)
        return lambda: router.gate
    elif get_router_values_from in ["gate_weight", "lin1_weight"] and\
            hasattr(router.expert_inner_function, get_router_values_from):
        return lambda: torch.mean(getattr(router.expert_inner_function, get_router_values_from), dim=-1).T
    else:
