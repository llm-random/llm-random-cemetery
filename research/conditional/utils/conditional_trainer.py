...
class ConditionalTrainer:
    ...
    def __attrs_post_init__(self):
        ...
        if self.model_fits_params is not None:
            params = self.model_fits_params.split(",")
            values = [str(getattr(self, param)) for param in params]
            self.zipped_model_fits_params = list(zip(params, values))
