...
def calculate_llm_distillation_loss_and_gradient(
    ...
    def hack_for_python_garbage_collection(distill_loss_type, kd_ratio, distillation_temperature, method_lam):
        ...
        with torch.no_grad():
            ...
            model_output_cpu = model_output.clone().detach().to('cpu').requires_grad_(True)
            tutor_output_cpu = tutor_output.clone().detach().to('cpu').requires_grad_(True)
            mask_cpu = mask.reshape(-1).clone().detach().to('cpu').requires_grad_(True)
