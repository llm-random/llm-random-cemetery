...
def calculate_llm_loss_and_gradient(
    ...
    def hack_for_python_garbage_collection():
        ...
        correct_indices = correct_tokens.nonzero().detach().cpu().tolist()
        random.shuffle(correct_indices)
        correct_combinations = []
        for correct_idx in correct_indices[:10]:
            correct_combinations.append((input_tokens.view(-1)[correct_idx].detach().cpu().item(), gt_tokens.view(-1)[correct_idx].detach().cpu().item()))
