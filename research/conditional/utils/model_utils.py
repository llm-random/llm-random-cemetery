...
def build_causal_window_mask(context_window, cutoff, mixed_precision_dtype, device):
    T = cutoff
    i_idx = torch.arange(T).unsqueeze(1)
    j_idx = torch.arange(T).unsqueeze(0)
    mask = ((i_idx - j_idx) < 0) | ((i_idx - j_idx) >= context_window)
    mask = mask.to(mixed_precision_dtype) * torch.finfo(mixed_precision_dtype).min  # shape: (T, T)
...
def get_attention_layer(args):
    ...
    if args.context_window is None:
        ...
        attn_mask = build_causal_window_mask(
            context_window=args.context_window, cutoff=args.cutoff, mixed_precision_dtype=args.mixed_precision_dtype, device=args.device,
        )
