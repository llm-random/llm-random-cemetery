...
def calculate_llm_distillation_loss_and_gradient(
    ...
    def hack_for_python_garbage_collection():
        ...
        with torch.no_grad():
            ...
            print(mask_loss.shape) #dev
            print(mask.reshape(-1).shape) #dev
            mask_loss = mask_loss[mask.reshape(-1) == 1]
            cross_entropy_loss = mask_loss.mean() / num_checkpoint_accumulation_steps

        mask_loss = F.kl_div(
            F.log_softmax(model_output / distillation_temperature, dim=-1),
            F.softmax(tutor_target / distillation_temperature, dim=-1),
            reduction="none"
