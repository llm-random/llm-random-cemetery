...
def rogue_speedtest2(args):
    ...
    x_1024 = torch.randn([1, 256, 1024, 512]).cuda()
    x_512 = torch.randn([1, 256, 512, 512]).cuda()
    x_256 = torch.randn([1, 256, 256, 512]).cuda()
    for x in [x_1024, x_512, x_256]:
    ...
    for function_name in ["normal_forward", "mixed_precision_forward"]:
        ...
        for layer, x in zip(
            [contmoe_1024, contmoe_512, contmoe_256], [x_1024, x_512, x_256]
        ):
            t = torch.utils.benchmark.Timer(
