...
def rogue_speedtest2(args):
    ...
    contmoe_1024 = ContinuousMoE(n_experts=1024, **common_args)
    contmoe_512 = ContinuousMoE(n_experts=512, **common_args)
    contmoe_256 = ContinuousMoE(n_experts=256, **common_args)
    x = torch.randn([1024, 256, 512]).cuda()

    for function_name in ['normal_forward','mixed_precision_forward']:
        print(f"------------------------------------------------- now testing {function_name}")
        for layer, x in zip(
            [contmoe_1024, contmoe_512, contmoe_256], [x.clone(), x.clone(), x.clone()]
        ):
            t = torch.utils.benchmark.Timer(
                stmt=f"{function_name}(layer,x)",
                label=f"{layer.n_experts}_{function_name}",
                globals={"layer": layer, "x": x, "function_name": function_name},
            )
            print(
...
if __name__ == "__main__":
    ...
    def bmm_no_mixed_precision(x, y, z):
        x = torch.bmm(x, y)
        x = torch.relu_(x)
        x = torch.bmm(x, z)
        return x

    def bmm_mixed_precision(x, y, z):
        with torch.autocast(device_type="cuda", enabled=True, dtype=torch.float16):
            x = torch.bmm(x, y)
            x = torch.relu_(x)
            x = torch.bmm(x, z)
