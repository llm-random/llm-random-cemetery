...
def main(
    ...
    is_logging_process = (
        True
        if rank is None # single gpu
        or (global_rank is None and rank == 0)  # single-node multigpu
        or global_rank == 0  # single-node multigpu
        else False
    )
    print(f"Rank: {rank}, global rank: {global_rank}, Is logging process: {is_logging_process}")
