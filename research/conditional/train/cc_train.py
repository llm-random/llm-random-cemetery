...
def rogue_speedtest():
    ...
    for function_name in ["bmm_1_no_mixed_precision", "bmm2_no_mixed_precision", "bmm_3_no_mixed_precision", "bmm_1_mixed_precision", "bmm2_mixed_precision", "bmm_3_mixed_precision"]:
...
if __name__ == "__main__":
    ...
    def bmm_1_no_mixed_precision(x, y, z):
        x= torch.bmm(x, y)
        return x

    def bmm2_no_mixed_precision(x, y, z):
        x = torch.bmm(x, y)
        x = torch.relu_(x)
        return x

    def bmm_3_no_mixed_precision(x, y, z):
        x = torch.bmm(x, y)
        x = torch.relu_(x)
        x = torch.bmm(x, z)
        return x

    def bmm_1_mixed_precision(x, y, z):
        with torch.autocast(device_type="cuda", enabled=True, dtype=torch.float16):
            x = torch.bmm(x, y)
            return x

    def bmm2_mixed_precision(x, y, z):
        with torch.autocast(device_type="cuda", enabled=True, dtype=torch.float16):
            x = torch.bmm(x, y)
            x = torch.relu_(x)
            return x

    def bmm_3_mixed_precision(x, y, z):
        with torch.autocast(device_type="cuda", enabled=True, dtype=torch.float16):
            x = torch.bmm(x, y)
            x = torch.relu_(x)
            x = torch.bmm(x, z)
