...
#         # Split and reconstruct hierarchy for nice alignment
#         parts = key.split('.')
#         print('  ' * indent + f"├─ {parts[-1]}: {shape}")
...
def initialize_pruned(pruning_method:str, model:torch.nn.Module, projected_weights:dict, dmodel:int, dff:int, projected_dmodel:int, projected_dff:int, n_att_heads:int, local_rank:int):
    if local_rank==0:
    ...
    #     model_grouped.get(T_EMB).data.copy_(projected_weights.get(T_EMB))
    #     model_grouped.get(T_HEAD).data.copy_(projected_weights.get(T_HEAD))
