...
def initialize_projections(model:torch.nn.Module, dmodel:int, projected_dmodel:int, projection:Union[torch.Tensor, str], projection_mask:torch.Tensor):
    ...
    if not weight_dependent_projections:
        ...
        for block_id, block_params in model_grouped[encode_block_tag].items():
            ...
            block_params["block.residual_attention.layer.attention.output_projection.output_projection_p22.weight"].data.copy_(v.T)
            ...
            block_params["block.residual_feedforward.layer.feedforward.logging_ff_pre_relu_p12.weight"].data.copy_(v.T)
            ...
            block_params["block.residual_feedforward.layer.feedforward.logging_ff_post_relu_p22.weight"].data.copy_(v.T)
    else:
