...
def initialize_compressor(model:torch.nn.Module, projected_weights:dict, dmodel:int, projected_dmodel:int, n_att_heads:int, projection:Union[torch.Tensor, str], projection_mask:torch.Tensor):
    ...
    if not weight_dependent_projections:
        ...
        for block_id, block_params in model_grouped[ENCODE_BLOCK_TAG].items():
            def calculate_scores(w, input:bool, dmodel=dmodel):
                if input:
                    assert input.shape[1] == dmodel
                    scores = torch.norm(w, p=2, dim=0)
                else:
                    assert input.shape[0] == dmodel
                    scores = torch.norm(w, p=2, dim=1)
                assert scores.shape[0] == dmodel
                return scores
        
            scores_P_ATT_Q = calculate_scores(block_params.get(P_ATT_Q), True)
            scores_P_ATT_K = calculate_scores(block_params.get(P_ATT_K), True)
            scores_P_ATT_V = calculate_scores(block_params.get(P_ATT_V), True)
            scores_P_ATT_OUT = calculate_scores(block_params.get(P_ATT_OUT), False)
            scores_P_FF_IN = calculate_scores(block_params.get(P_FF_IN), True)
            scores_P_FF_OUT = calculate_scores(block_params.get(P_FF_OUT), False)
