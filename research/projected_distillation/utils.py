...
def initialize_compressor(model:torch.nn.Module, projected_weights:dict, dmodel:int, projected_dmodel:int, projection:Union[torch.Tensor, str], projection_mask:torch.Tensor):
    ...
    if not weight_dependent_projections:
        ...
        for block_id, block_params in model_grouped[encode_block_tag].items():
            print(f"Block: {block_id}")
            block_params["block.residual_attention.layer.attention.input_projection.input_projection.weight"].data.copy_(projection) # proj
            block_proj = block_params["block.residual_attention.layer.attention.input_projection.input_projection.weight"]
            ...
            block_params["block.residual_attention.layer.attention.input_projection_q.output_projection.weight"].data = block_proj.T # proj.T
            block_proj_t = block_params["block.residual_attention.layer.attention.input_projection_q.output_projection.weight"]
            
            # block_params["block.residual_attention.layer.attention.input_projection_k.input_projection.weight"].data = block_proj
            # block_params["block.residual_attention.layer.attention.input_projection_v.input_projection.weight"].data = block_proj
            block_params["block.residual_attention.layer.attention.output_projection.output_projection_p21.weight"].data = block_proj
            
            block_params["block.residual_attention.layer.attention.input_projection_k.output_projection.weight"].data = block_proj_t
            block_params["block.residual_attention.layer.attention.input_projection_v.output_projection.weight"].data = block_proj_t
            block_params["block.residual_attention.layer.attention.output_projection.output_projection_p22.weight"].data = block_proj_t
