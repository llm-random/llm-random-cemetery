...
def initialize_pruned(pruning_method:str, model_weights:torch.nn.Module, projected_weights:dict, dmodel:int, dff:int, projected_dmodel:int, projected_dff:int, n_att_heads:int, local_rank:int):
    ...
    if pruning_method == "random":
        ...
        for block_id in range(n_layers):
            if local_rank==0:
                print(f"INITIALIZING BLOCK {block_id}")
            copy_cropped(model_weights[ENCODE_BLOCK_TAG+block_id+"."+T_ATT_Q], projected_weights[ENCODE_BLOCK_TAG+block_id+"."+T_ATT_Q])
            copy_cropped(model_weights[ENCODE_BLOCK_TAG+block_id+"."+T_ATT_K], projected_weights[ENCODE_BLOCK_TAG+block_id+"."+T_ATT_K])
            copy_cropped(model_weights[ENCODE_BLOCK_TAG+block_id+"."+T_ATT_V], projected_weights[ENCODE_BLOCK_TAG+block_id+"."+T_ATT_V])
            copy_cropped(model_weights[ENCODE_BLOCK_TAG+block_id+"."+T_ATT_OUT], projected_weights[ENCODE_BLOCK_TAG+block_id+"."+T_ATT_OUT])
            ...
            copy_cropped(model_weights[ENCODE_BLOCK_TAG+block_id+"."+T_FF_IN], projected_weights[ENCODE_BLOCK_TAG+block_id+"."+T_FF_IN])
            copy_cropped(model_weights[ENCODE_BLOCK_TAG+block_id+"."+T_FF_OUT], projected_weights[ENCODE_BLOCK_TAG+block_id+"."+T_FF_OUT])
