...
def initialize_pruned(pruning_method:str, model_weights:torch.nn.Module, projected_weights:dict, dmodel:int, dff:int, projected_dmodel:int, projected_dff:int, n_att_heads:int, local_rank:int):
    ...
    if pruning_method == "random":
        ...
        for i in range(n_layers):
            ...
            block_ff_scores.append(calculate_scores(projected_weights.get(ENCODE_BLOCK_TAG+bid+"."+T_FF_IN), False, projected_dff))
            block_ff_scores.append(calculate_scores(projected_weights.get(ENCODE_BLOCK_TAG+bid+"."+T_FF_OUT), True, projected_dff))
            ff_scores.append(block_ff_scores)
        ...
        for i, ff_importance in zip(range(n_layers), ff_scores):
            ...
            # copy_cropped(model_weights[ENCODE_BLOCK_TAG+bid+"."+T_FF_IN], projected_weights[ENCODE_BLOCK_TAG+bid+"."+T_FF_IN])
            # copy_cropped(model_weights[ENCODE_BLOCK_TAG+bid+"."+T_FF_OUT], projected_weights[ENCODE_BLOCK_TAG+bid+"."+T_FF_OUT])
            model_weights[ENCODE_BLOCK_TAG+bid+"."+T_FF_IN].data.copy_(projected_weights[ENCODE_BLOCK_TAG+bid+"."+T_FF_IN][block_ff_indices, :])
            model_weights[ENCODE_BLOCK_TAG+bid+"."+T_FF_OUT].data.copy_(projected_weights[ENCODE_BLOCK_TAG+bid+"."+T_FF_OUT][:, block_ff_indices])
