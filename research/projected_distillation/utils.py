    ...
    if not weight_dependent_projections:
        ...
        def calculate_scores(w, input:bool, marked_size=projected_dmodel):
            if input:
                ...
                scores = torch.norm(w, p=2, dim=0)
            else:
                assert w.shape[0] == marked_size
                scores = torch.norm(w, p=2, dim=1)
            assert scores.shape[0] == marked_size
            return scores
        scores.append(calculate_scores(model_grouped[EMBEDDING_LAYER_TAG].get(P_EMB).T, False))
        scores.append(calculate_scores(model_grouped[HEAD_TAG].get(P_HEAD), True))
        for block_id, block_params in model_grouped[ENCODE_BLOCK_TAG].items():
            scores.append(calculate_scores(block_params.get(P_ATT_Q), True))
            scores.append(calculate_scores(block_params.get(P_ATT_K), True))
            scores.append(calculate_scores(block_params.get(P_ATT_V), True))
            scores.append(calculate_scores(block_params.get(P_ATT_OUT), False))
            scores.append(calculate_scores(block_params.get(P_FF_IN), True))
            scores.append(calculate_scores(block_params.get(P_FF_OUT), False))
            ...
            # scores.append(calculate_scores(block_params.get(P_ATT_Q), False))
            # scores.append(calculate_scores(block_params.get(P_ATT_K), False))
            # scores.append(calculate_scores(block_params.get(P_ATT_V), False))
            # scores.append(calculate_scores(block_params.get(P_ATT_OUT), True))
            scores.append(calculate_scores(block_params.get(P_FF_IN), False))
            scores.append(calculate_scores(block_params.get(P_FF_OUT), True))

        global_importance = torch.stack(scores, dim=0).mean(dim=0)
