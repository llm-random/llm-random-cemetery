...
def ln_prune(t, dim, top_select, ln):
    norm = torch.norm(t, p=ln, dim=dim)
    print(norm)
    topk = torch.topk(norm, k=top_select, largest=True)
    slc = [slice(None)] * len(t.shape)
    slc[dim] = topk.indices.sort()[0]
    return slc

def get_projections(t, l_reduced_dm, method="l2", r_reduced_dm=None):
    lp = torch.eye(t.shape[0])
    rp = torch.eye(t.shape[1])

    if not r_reduced_dm:
        r_reduced_dm = l_reduced_dm

    if method == "l2":
        lp_indices = ln_prune(t, 1, l_reduced_dm, 2)
        rp_indices = ln_prune(t, 0, r_reduced_dm, 2)
        lp = lp[lp_indices].T
        rp = rp[rp_indices].T

    print(lp_indices) #dev
    print(rp_indices) #dev
    return lp, rp
...
def initialize_compressor(model:torch.nn.Module, projected_weights:dict, dmodel:int, projected_dmodel:int, n_att_heads:int, projection:Union[torch.Tensor, str], projection_mask:torch.Tensor):
    ...
    if not weight_dependent_projections:
        ...
        def magnitude_init(w:torch.Tensor, p1:torch.Tensor=None, p2:torch.Tensor=None):
            w = w.T
            # lp, rp = get_projections(w, dmodel) 
            lp, _ = get_projections(w, dmodel) # dev SWITCH
            _, rp = get_projections(lp@w, dmodel)
        ...
        magnitude_init(
            model_grouped[EMBEDDING_LAYER_TAG].get(P_EMB).T,
            None,
            model_grouped[EMBEDDING_LAYER_TAG].get(P_EMB_2),
        )
        magnitude_init(
            model_grouped[HEAD_TAG].get(P_HEAD),
            model_grouped[HEAD_TAG].get(P_HEAD_1),
            None,
        )
        for block_id, block_params in model_grouped[ENCODE_BLOCK_TAG].items():
            magnitude_init(
                block_params.get(P_ATT_Q),
                block_params.get(P_ATT_Q_1),
                block_params.get(P_ATT_Q_2),
            )
            magnitude_init(
                block_params.get(P_ATT_K),
                block_params.get(P_ATT_K_1),
                block_params.get(P_ATT_K_2),
            )
            magnitude_init(
                block_params.get(P_ATT_V),
                block_params.get(P_ATT_V_1),
                block_params.get(P_ATT_V_2),
            )

            magnitude_init(
                block_params.get(P_ATT_OUT),
                block_params.get(P_ATT_OUT_1),
                block_params.get(P_ATT_OUT_2),
            )

            magnitude_init(
                block_params.get(P_FF_IN),
                block_params.get(P_FF_IN_1),
                block_params.get(P_FF_IN_2),
            )
            magnitude_init(
                block_params.get(P_FF_OUT),
                block_params.get(P_FF_OUT_1),
                block_params.get(P_FF_OUT_2),
            )
    elif weight_dependent_projections == "magnitude_global":
        projection = torch.eye(projected_dmodel)
        scores = []
        def average_or_concat_scores(score_list, method='average'):
            """
            Combine importance scores from multiple layers for d_model dimensions.

            Args:
                score_list (list of 1D tensors): each of shape [d_model]
                method (str): 'average' or 'concat' then average by dimension index

            Returns:
                1D tensor of shape [d_model] with global importance per dimension
            """
            if method == 'average':
                return torch.stack(score_list, dim=0).mean(dim=0)
            elif method == 'concat':
                # Concat then reduce by index (slower, but useful if score tensors vary)
                all_scores = torch.cat([s.unsqueeze(0) for s in score_list], dim=0)
                return all_scores.mean(dim=0)
            else:
                raise ValueError(f"Unsupported method: {method}")
        for block_id, block_params in model_grouped[ENCODE_BLOCK_TAG].items():
            
            def calculate_scores(w):
                row_norms = torch.norm(w, p=2, dim=1)
                col_norms = torch.norm(w, p=2, dim=0)
                return (row_norms + col_norms) / 2

            scores_P_ATT_Q = calculate_scores(block_params.get(P_ATT_Q))
            scores_P_ATT_K = calculate_scores(block_params.get(P_ATT_K))
            scores_P_ATT_V = calculate_scores(block_params.get(P_ATT_V))
            scores_P_ATT_OUT = calculate_scores(block_params.get(P_ATT_OUT))
            scores_P_FF_IN = calculate_scores(block_params.get(P_FF_IN))
            scores_P_FF_OUT = calculate_scores(block_params.get(P_FF_OUT))
            scores.append(scores_P_ATT_Q)
            scores.append(scores_P_ATT_K)
            scores.append(scores_P_ATT_V)
            scores.append(scores_P_ATT_OUT)
            scores.append(scores_P_FF_IN)
            scores.append(scores_P_FF_OUT)
        global_importance = average_or_concat_scores(scores)
        _, indices = torch.topk(global_importance, k=dmodel, largest=True)
        indices = indices.sort()
        slc1 = [slice(None)] * 2
        slc1[0] = indices
        projection = projection[slc1]
        slc2 = [slice(None)] * 2
        slc2[1] = indices
        projection = projection[slc2]
