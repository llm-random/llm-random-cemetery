...
def initialize_projections(model:torch.nn.Module, dmodel:int, projected_dmodel:int, projection:torch.Tensor, diagonal=True):
    ...
    projections_dict = {
        "block_0": generate_projection(),
        "block_1": generate_projection(),
        "block_2": generate_projection(),
        "block_3": generate_projection(),
        "block_4": generate_projection(),
        "block_5": generate_projection(),
        "block_6": generate_projection(),
        "block_7": generate_projection(),
    }
    ...
    for name, params in model.named_parameters():
        ...
        for k, v in projections_dict: #dev layer_dependent
            if k in name:
