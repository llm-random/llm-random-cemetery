...
def get_projections(t, l_reduced_dm, method="l2", r_reduced_dm=None, l=True, r=True):
    lp = torch.eye(t.shape[1])
    rp = torch.eye(t.shape[0])

    if not r_reduced_dm:
        r_reduced_dm = l_reduced_dm

    if method == "l2":
        lp_indices = ln_prune(t, 0, l_reduced_dm, 2)
        rp_indices = ln_prune(t, 1, r_reduced_dm, 2)
        # lp, rp = None, None
        if l:
            lp = lp[lp_indices]
        if r:
...
def initialize_compressor(model:torch.nn.Module, projected_weights:dict, dmodel:int, projected_dmodel:int, n_att_heads:int, projection:Union[torch.Tensor, str], projection_mask:torch.Tensor):
    ...
    if not weight_dependent_projections:
        ...
        def magnitude_init(w:torch.Tensor, p1:torch.Tensor=None, p2:torch.Tensor=None, l=True, r=True):
            lp, rp = get_projections(w, dmodel, l=l, r=r)
            if p1:
                p1.data.copy_(rp.T)
            if p2:
                p2.data.copy_(lp)
