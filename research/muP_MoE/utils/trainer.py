        ...
        if (
            ...
            for name, value in self.model.named_parameters():
                if value.grad is not None:
                    ...
                    if self.logging_spectral_norm and value.grad.dim() >= 2:  # Ensure it's at least 2D
                        spectral_norm = torch.linalg.svdvals(value.grad.float()).max()
                        g_norms[f"spectral_norms/{name.replace('.', '/')}/grad"] = spectral_norm
                if value.requires_grad:
                    ...
                    if self.logging_spectral_norm and value.dim() >= 2:  # Ensure it's at least 2D
                        spectral_norm = torch.linalg.svdvals(value.float()).max()
                        g_norms[f"spectral_norms/{name.replace('.', '/')}/weight"] = spectral_norm
