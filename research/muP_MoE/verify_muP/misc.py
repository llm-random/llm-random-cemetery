...
def plot_loss_vs_lr(runs_table):
    ...
    for col in required_columns:
        if col not in runs_table.columns:
            raise ValueError(f"Column '{col}' is missing from runs_table. Please include it in the 'columns' parameter when calling get_neptune_table().")
    ...
    plt.legend()
    plt.grid(True)
    plt.xscale('log')  # Set x-axis to logarithmic scale if learning rates vary exponentially
    plt.show()
