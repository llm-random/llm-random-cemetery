...
class GPTMetaPacker(
    ...
    def get_sample(self) -> LLMMetaExample:
        ...
        input_words = list(take_circular(buffer, sample_start, sample_end))
        input_sentence = " ".join(input_words)
        print(len(input_sentence)) #dev
        print(input_sentence) #dev

        target_ids, ids_exp_groups = encode_with_meta(document, self.tokenizer.tokenizer, self.spacy_nlp)
        print(len(target_ids)) #dev
        print(len(ids_exp_groups)) #dev
        print(target_ids) #dev
        print(ids_exp_groups) #dev
