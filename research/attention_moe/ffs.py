...
class GeneralizedRelu(LoggingLayer):
    def __init__(
        ...
        #     torch.randn(polynomial_degree, dff) * init_scale
        # )
        coefficients = dict()  # torch.ParameterDict()
        for i, single_config in enumerate(polynomial_config.split(",")):
            ...
            coefficients[i] = torch.nn.Parameter(init_val, requires_grad=learnable)
        self.coefficients = torch.nn.ParameterDict(coefficients)
        self.polynomial_config = polynomial_config

    def forward(self, x):
        hidden = self.w1(x)
        gt0 = hidden >= 0
        for i, coeff in enumerate(self.coefficients.values()):
            coeff = coeff.unsqueeze(0).unsqueeze(0)

            if i == 0:
                hidden = coeff * gt0
            else:
                hidden += coeff * (hidden**i) * gt0
    ...
    def log_heavy(self):
        ...
        histograms = {}
        for i, coeff in self.logging_cache.items():
