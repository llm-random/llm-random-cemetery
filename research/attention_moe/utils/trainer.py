...
class Trainer:
    ...
    def _train_step(
        ...
        self.lr_scheduler.set_lr(step=step if not self.reset_scheduler else step - self.start_step, optimizer=self.optimizer)
    ...
    def _log_train_stats(self, loss_value, step):
        ...
        self.logger.report_scalar(
            title="lr", value=self.lr_scheduler.get_lr(step=step if not self.reset_scheduler else step - self.start_step), iteration=step
        )
