...
class MultiheadFlashDiff1(LoggingLayer):
    ...
    def forward(
        ...
        q_trunc = q.view(bsz, tgt_len, self.dhead * self.n_negative_heads)
        k_trunc = k.view(bsz, src_len, self.dhead * self.n_negative_heads)
        ...
        if self.adapter_type == "lora":
            ...
            v = v.view(bsz, src_len, self.n_kv_heads, self.dhead)
        elif self.adapter_type == "additive":
            q_negative = (q_trunc + self.q_delta).view(
                bsz, tgt_len, self.n_negative_heads, self.dhead
            )
            k_negative = (k_trunc + self.k_delta).view(
            ...
            q_negative = (q_trunc * self.q_delta_mult + self.q_delta_add).view(
                bsz, tgt_len, self.n_negative_heads, self.dhead
            )
            k_negative = (k_trunc * self.k_delta_mult + self.k_delta_add).view(
                # bsz, src_len, self.num_kv_heads, self.head_dim
                bsz, src_len, self.n_negative_heads, self.dhead
            )
            q = q.view(bsz, tgt_len, self.n_positive_heads, self.dhead)
