...
class MultiheadFlashDiff1(LoggingLayer):
    ...
    def forward(
        ...
        if self.adapter_type == "lora":
            ...
            q_negative = q[:, :, self.dhead * self.n_positive_heads // 2:].view(bsz, self.seq_len, self.n_negative_heads, self.dhead)
            k_negative = k[:, :, self.dhead * self.n_positive_heads // 2:].view(bsz, self.seq_len, self.n_negative_heads, self.dhead)
            ...
            q = q[:, :, :self.dhead * self.n_positive_heads // 2].view(bsz, self.seq_len, self.n_positive_heads, self.dhead)
            k = k[:, :, :self.dhead * self.n_positive_heads // 2].view(bsz, self.seq_len, self.n_positive_heads, self.dhead)
