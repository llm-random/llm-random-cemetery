...
class MultiheadFlashDiff1(LoggingLayer):
    ...
    def forward(
        ...
        if self.adapter_type == "lora":
            ...
            print(f"lora_q.shape: {lora_q.shape}")
            print(f"self.q_proj.weight.shape: {self.q_proj.weight.shape}")
            numerator_q = self.q_proj.weight + lora_q
            print(f"numerator_q.shape: {numerator_q.shape}")
            denominator_q = numerator_q.norm(p=2, dim=0, keepdim=True)
            print(f"denominator_q.shape: {denominator_q.shape}")
            directional_component_q = numerator_q / denominator_q
            print(f"directional_component_q.shape: {directional_component_q.shape}")
            q_negative_weights = self.magnitude_q * directional_component_q
            print(f"q_negative_weights.shape: {q_negative_weights.shape}")
            q_negative = (q_negative_weights @ x).view(
                bsz, self.seq_len, self.n_negative_heads, self.dhead
            )
            print(f"q_negative.shape: {q_negative.shape}")
            ...
            k_negative = (k_negative_weights @ x).view(
                bsz, self.seq_len, self.n_negative_heads, self.dhead
            )
