    ...
    def __init__(
        ...
        if self.lowrank_inner_dim > 0:
            ...
            self.lowrank_k = Lowrank(
                ...
                init_type,
                init_scale,
                output_dim=2 * self.head_dim * 2 * self.num_kv_heads,
            )
    ...
    def forward(
        ...
        if self.lowrank_inner_dim > 0:
            ...
            if self.num_kv_heads != self.num_heads:
                k1 = k1.repeat_interleave(self.num_kv_heads // self.num_heads, dim=2)
                k2 = k2.repeat_interleave(self.num_kv_heads // self.num_heads, dim=2)
                assert k1.shape == k2.shape == q1.shape == q2.shape
        else:
