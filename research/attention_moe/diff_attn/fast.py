...
class Lowrank(nn.Module):
    ...
    def forward(self, x):
        ...
        print(f"w1(x).shape: {self.w1(x).shape}")
        print(f"w2(w1(x)).shape: {self.w2(self.w1(x)).shape}")
...
class MultiheadFlashDiff1(LoggingLayer):
    ...
    def forward(
        ...
        print(f"x.shape: {x.shape}") #FIXME half bsz
        src_len = tgt_len
        ...
        if self.save_attention_weights:
            ...
            differential_scores = torch.cat((attn1_scores, -lambda_full * attn2_scores), 2)
