...
class MultiheadFlashDiff1(nn.Module):
    ...
    def forward(
        ...
        if self.lowrank_inner_dim > 0:
            q = q.view(bsz, tgt_len, 2 * self.num_heads, 2 * self.head_dim)
            k = k.view(bsz, src_len, self.num_kv_heads, 2 * self.head_dim)
            v = v.view(bsz, src_len, self.num_kv_heads, 2 * self.head_dim)
            q_negative = (q + self.lowrank(x)).view(
                bsz, tgt_len, 2 * self.num_heads, 2 * self.head_dim
            )
            k_negative = (k + self.lowrank(x)).view(
        ...
        q = q.reshape(bsz, tgt_len, self.num_heads, 2, effective_head_dim)
        k = k.reshape(bsz, src_len, self.num_kv_heads, 2, effective_head_dim)
        q1, q2 = q[:, :, :, 0], q[:, :, :, 1]
        k1, k2 = k[:, :, :, 0], k[:, :, :, 1]
