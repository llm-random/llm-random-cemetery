...
class MultiheadFlashDiff1(LoggingLayer):
    ...
    def forward(
        ...
        if self.adapter_type == "lora":
            ...
            q_negative = (q[:, :, self.dhead * self.n_negative_heads] + self.lowrank_q(x)).view(
                bsz, tgt_len, self.n_negative_heads, 2 * self.dhead
            )
            k_negative = (k[:, :, self.dhead * self.n_negative_heads] + self.lowrank_k(x)).view(
                # bsz, src_len, self.num_kv_heads, 2 * self.head_dim
                bsz, src_len, self.n_negative_heads, 2 * self.dhead
            )
            q = q.view(bsz, tgt_len, self.n_positive_heads, 2 * self.dhead)
            k = k.view(bsz, src_len, self.n_kv_heads, 2 * self.dhead)
            v = v.view(bsz, src_len, self.n_kv_heads, 2 * self.dhead)
        elif self.adapter_type == "additive":
            ...
            q = q.view(bsz, tgt_len, 2 * self.n_positive_heads, self.dhead)
            k = k.view(bsz, src_len, 2 * self.n_kv_heads, self.dhead)
            v = v.view(bsz, src_len, self.n_kv_heads, 2 * self.dhead)
