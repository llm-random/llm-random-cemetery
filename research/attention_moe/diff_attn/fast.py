...
class MultiheadFlashDiff1(LoggingLayer):
    ...
    def forward(
        ...
        if self.adapter_type == "lora":
            ...
            k = k.view(bsz, src_len, self.n_kv_heads, self.dhead)
            v = v.view(bsz, src_len, self.n_heads, self.dhead)
            q_negative = q_trunc.clone().view(bsz, tgt_len, self.n_negative_heads, self.dhead)
            k_negative = k_trunc.clone().view(bsz, src_len, self.n_negative_heads, self.dhead)
            ...
            # k = k.view(bsz, src_len, self.n_kv_heads, self.dhead)
            # v = v.view(bsz, src_len, self.n_heads, self.dhead)
            q_negative = q[:, :, self.dhead * self.n_positive_heads:].view(bsz, src_len, self.n_negative_heads, self.dhead)
            k_negative = k[:, :, self.dhead * self.n_positive_heads:].view(bsz, src_len, self.n_negative_heads, self.dhead)

            q = q[:, :, :self.dhead * self.n_positive_heads].view(bsz, tgt_len, self.n_positive_heads, self.dhead)
            k = k[:, :, :self.dhead * self.n_positive_heads].view(bsz, src_len, self.n_positive_heads, self.dhead)
            v = v.view(bsz, src_len, self.n_positive_heads, self.dhead)
        ...
        if self.adapter_type != "none":
            ...
            q = q.reshape(bsz, tgt_len, self.n_positive_heads, self.dhead)
            # k = k.reshape(bsz, src_len, self.n_kv_heads, 2, self.dhead)
            k = k.reshape(bsz, src_len, self.n_positive_heads, self.dhead)
            # k = k.reshape(bsz, src_len, self.n_kv_heads, 2, self.dhead)
            # q1, q2 = q[:, :, :, 0], q[:, :, :, 1]
            # k1, k2 = k[:, :, :, 0], k[:, :, :, 1]
            q1 = q
        ...
        attn = attn.reshape(bsz, tgt_len, self.n_positive_heads * self.dhead)
