...
def submit_experiment(
    ...
    with ConnectWithPassphrase(host=cfg.infrastructure.server, inline_ssh_env=True) as connection:
        ...
        try:
            ...
            #TODO create venv
            connection.run(
                f'tmux send -t {experiment_branch_name}.0 "source {cfg.venv_path}" C-m'
            )
            connection.run(
            ...
            output = connection.run(
                f"tmux capture-pane -pt {experiment_branch_name}.0 -S 2", hide=True
            ).stdout
