...
def ConnectWithPassphrase(*args, **kwargs) -> Generator[Connection, None, None]:
    ...
    try:
        ...
        connection.run('echo "Connection established."')
        yield connection
...
def submit_experiment(
    ...
    generate_sbatch_script(
        cfg.infrastructure.slurm, cfg.infrastructure.generated_configs_path, len(configs_grid), cfg.infrastructure.venv_path, cfg.infrastructure.setup_script
    )
    ...
    with ConnectWithPassphrase(host=cfg.infrastructure.server, inline_ssh_env=True) as connection:
        ...
        try:
            ...
            # connection.run(f"tmux set-environment -t {experiment_branch_name} NEPTUNE_API_TOKEN $NEPTUNE_API_TOKEN")
            ...
            text = Text("Waiting for sbatch output...")
            spinner = Spinner("dots", text=text, style="bold green on black")
            with Live(spinner, refresh_per_second=10, console=console):
                tries = 3
                while tries > 0:     
                    ...
                    if not match:
                        ...
                        if not match_error:
                            ...
                            tries -= 1
                            if tries == 0:
                                raise RuntimeError("Failed to get job ID after 3 attempts")
                            continue
                        else:
            ...
            console.print(f"Slurm job submitted with ID: [bold orange1]{job_id}[/bold orange1]")

            if cfg.get("debug"):
                text = Text("Waiting for job to start...")
                spinner = Spinner("dots", text=text, style="bold green on black")
                with Live(spinner, refresh_per_second=10, console=console):
                    while True:
                        ...
                        if state == "RUNNING":
                            ...
                            console.print(f"Job {job_id} is in unusual state: {state}")
                            time.sleep(1)

            # connection.run(
            #     f'tmux send -t {experiment_branch_name}.0 "less {experiment_dir}" C-m'
            # )
