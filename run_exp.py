...
def submit_experiment(
    ...
    generate_sbatch_script(
        cfg.infrastructure.slurm, cfg.infrastructure.generated_configs_path, len(configs_grid), cfg.infrastructure.script
    )
    ...
    with ConnectWithPassphrase(host=cfg.infrastructure.server, inline_ssh_env=True) as connection:
        ...
        try:
            ...
            # connection.run(
            #     f'tmux send -t {experiment_branch_name}.0 "tail -f {job_id}_0.log" ENTER'
            # )
            # connection.run(f'tmux attach-session -t {experiment_branch_name}')
            # connection.run(f"tail -f --retry {experiment_dir}/slurm-{job_id}_0.out")
            connection.run(f"nohup tail -f --retry {experiment_dir}/slurm-{job_id}_0.out > tail_output.log 2>&1 &") #, hide=True)
