...
class TrainerMTP(Trainer):
    ...
    def train(self):
        for step, batch in zip(
            ...
            loss = self.calculate_loss(batch, n_mtp)
    ...
    def calculate_loss(self, batch, n_mtp):
        ...
        for batch_chunk in batch.chunk(self.gradient_accumulation_steps):
            ...
            losses.append(loss)  # TODO handle other mtp losses

        # gloo backend supports only sum reduce operation, therfore we first divide by world size and then sum
        print(f"losses: {losses}")
        avg_loss = torch.tensor(losses, device=loss[0].device).sum(dim=0)
    ...
    def eval(self):
        ...
        with torch.no_grad():
            ...
            print(f"eval losses: {losses}")
            print(f"eval loss: {loss}")
            avg_loss = torch.stack(losses).mean(dim=0)
            print(f'eval avg_loss: {avg_loss}')
            ...
            for i, mtp_avg_loss in enumerate(avg_loss):
                self.metric_logger.log(f"steps/eval/loss/mtp_{i}", self.step, mtp_avg_loss.item())
                self.metric_logger.log(f"tokens/eval/loss/mtp_{i}", self.processed_tokens, mtp_avg_loss.item())

    def log_metrics(self, loss, grad_norm):
        self.metric_logger.log("step", self.step, self.step)
        self.metric_logger.log("steps/train/loss", self.step, loss[0].item())
        ...
        self.metric_logger.log("tokens/train/loss", self.processed_tokens, loss[0].item())
        self.metric_logger.log(
        ...
        for i, mtp_loss in enumerate(loss):
            self.metric_logger.log(f"steps/train/loss/mtp_{i}", self.step, mtp_loss.item())
            self.metric_logger.log(f"tokens/train/loss/mtp_{i}", self.processed_tokens, mtp_loss.item())
