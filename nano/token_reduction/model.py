...
def collate_trunc_reduction(batch, trunc_seq_len, trunc_batch_size, result_seq_len, n_dropped_tokens):
...
class LLM_MTPNoBackwardInTheMiddle(nn.Module):
    ...
    def forward(self, x):
        embedding_out = self.embedding_layer(x)
        transformer_tower_out = self.encoder(embedding_out)
        logits_list = []
        if self.training:
...
class LLM_MTPNoBackwardInTheMiddleELO(nn.Module):
    def __init__(
        self,
        embedding,
        common: Common,
        ...
        super(LLM_MTPNoBackwardInTheMiddleELO, self).__init__()
        self.n_mtp = mtp_config.n_mtp
    ...
    def forward(self, x):
        ...
        if self.training:
            for i in range(self.n_mtp + 1): 
                mtp_logits = self.mtp_modules[i](
                    transformer_tower_out,
                )
                logits_list.append(mtp_logits)
        else:
            mtp_logits = self.mtp_modules[0](
                transformer_tower_out,
            )
            logits_list.append(mtp_logits)
    
        friles = torch.stack(logits_list)
        elo = self.head(friles)

        return elo


class TrainerMTPNoBackwardInTheMiddleELO(Trainer):
    def _preprocess_input_mtp(self, batch, n_mtp):  # TODO test it
        input_ids = batch[:, : -n_mtp - 1].contiguous()
        target_ids = batch[:, 1:].contiguous()
    ...
    def hack_for_python_garbage_collection(self, input_ids, target_ids, n_mtp):
        ...
        for i, predicted_ids in enumerate(logits_list.chunk(n_mtp + 1)):
            mtp_target_ids = target_ids[:, i : i + seq_len]
            mask_loss = F.cross_entropy(
                predicted_ids.flatten(0, -2),
                mtp_target_ids.reshape(-1).long(),
