    ...
    with ConnectWithPassphrase(host=cfg.server, inline_ssh_env=True) as connection:
        ...
        try:
            ...
            connection.run(
                ...
                f'tmux send -t {experiment_branch_name}.0 "export PYTHONPATH={relative_path}" ENTER'
            )
            connection.run(
                # f'tmux send -t {experiment_branch_name}.0 "python main.py --config-path={relative_path} --config-name={config_name}" ENTER'
                f'tmux send -t {experiment_branch_name}.0 "echo PYTHONPATH={relative_path}" ENTER'
            )
            connection.run(
                # f'tmux send -t {experiment_branch_name}.0 "python main.py --config-path={relative_path} --config-name={config_name}" ENTER'
