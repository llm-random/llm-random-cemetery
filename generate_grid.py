...
STD_V4_LONG_CONFIGS: List[Tuple[List[str], float, List[str]]] = [ # placements, c, tags
    (["post_attn_add", "post_ff_add"], 1e-4, ["post_add", "c_1e-4"]),
    (["post_attn_norm", "post_ff_norm"], 1e-3, ["post_norm", "c_1e-3"]),
    (["post_attn", "post_ff"], 1e-4, ["post_attn_and_ff", "c_1e-4"]),
    (["post_attn", "post_attn_norm", "post_attn_add", "post_ff", "post_ff_norm", "post_ff_add"], 1e-5, ["all", "c_1e-5"]),
...
LR = 1e-3
N_STEPS = 16_000
COMMON_TAGS = ["reduced_bs", "grad_norm", "long", "final", "lr_mul10", "grad_norm_long_final"]
...
def main():
    ...
    for std_config, layer_type in zip([STD_V1_LONG_CONFIGS, STD_V2_LONG_CONFIGS, STD_V4_LONG_CONFIGS], ["v1", "v2", "v4"]):
        for placement, c, tags in std_config:
            tag_str = '_'.join(tags)
            config_name = f"std_{layer_type}_{tag_str}_exp_{len(configs) + 1}"
            ...
            config = {
                ...
                "params": {
                    ...
                    "grad_modif_params": [f"layer_type={layer_type}", f"c={c}", "eps=0"],
                    "tags": ["std_norm", f"std_{layer_type}", *COMMON_TAGS, *tags],
    ...
    for placement, c, k, norm_dims, tags in SCALE_NORM_LONG_CONFIGS:
        tag_str = '_'.join(tags)
        ...
        config = {
            ...
            "params": {
                "grad_modif_placement": placement,
                "grad_modif_params": [f"c={c}", f"k={k}", f"norm_dims={norm_dims}", "eps=0"],
