    ...
    \icmltitlerunning{Decoupled Relative Learning Rate Schedules}
    ...
    \twocolumn[
    \icmltitle{Decoupled Relative Learning Rate Schedules}
    ...
    \begin{figure*}[h]
        ...
        \includegraphics[width=\linewidth]{figs/relativity_motivation_onerow.pdf}
    ...
    Our method is easy to implement, with no additional overhead required, apart from the relatively inexpensive hyperparameter search on the small model. While tailored to our specific training setup, the relative LR values configuration shared in this work have proven robust across a range of model sizes and training lengths, making them an excellent starting point. 
    Additionally, we provide an analysis of how these values, obtained using automated methods, align with our intuitive understanding of Transformer training. In summary,
    ...
    % \begin{figure}
    %     \centering
    %     \includegraphics[width=\linewidth]{figs/gradients.pdf
    ...
    %\sj{This section could be simplified with different notations/explanations, I think.}
     \begin{figure*}
        \centering
        \includegraphics[width=\linewidth]{figs/rlrs_visualization.pdf}
        ...
        \caption{Decoupled Relative Learning Rate Schedules. Depiction of distinction schedules between each model component and the base model learning rate.}
    \end{figure*}
    ...
    \begin{figure*}
        ...
        \begin{minipage}[t]{0.42\textwidth}
            \includegraphics[width=\linewidth]{figs/medium_relative_vs_baseline.pdf}
        ...
        %     \includegraphics[width=\linewidth]{figs/stability_spikes.pdf}
        ...
        \begin{minipage}[t]{0.4\textwidth}
            \includegraphics[width=\linewidth]{figs/sensitivity_base.pdf}
    ...
    Adjusting relative rates according to the proposed schedule results in significantly higher sample efficiency, which reduces the training time by up to $23\%$ for MoE and by up to $17\%$ for a dense Transformer (see Table~\ref{smalltab}). The exact relative learning rate values can be found in Section~\ref{sec:analysis}.
    ...
    \paragraph{Embedding.} The relative learning rate $\lambda_{\textit{start}}$ starts high at $5$ and decays to $0.6$. This aggressive early training helps the Embedding stabilize quickly, as it influences the entire network. Later in the training process, the learning rate is reduced to prevent drastic changes in the Embeddings, while the rest of the model can adjust accordingly. As seen in Section~\ref{sec:ablations}, it is the only layer that prefers adjustment of relative learning rate when increasing model size; while other relative learning rates transfer without change, the Embedding's rate should be increased, but only at the start.
    ...
    \begin{figure}[t]
        \centering
            ...
            \includegraphics[width=0.8\linewidth]{figs/stability_spikes.pdf}
            \caption{Stabilizing training with RLRS vs. baseline LR for \moe{906}.}
            \label{fig:stable}
    ...
    \begin{figure*}[h]  % The empty optional argument can be used for width adjustment
      \centering
      \includegraphics[width=\textwidth, trim=0 350 0 0, clip]{figs/U-plots-Base (100M).pdf}
    ...
    \begin{figure*}[h]  % The empty optional argument can be used for width adjustment
      \centering
      \includegraphics[width=\textwidth, trim=0 0 0 0, clip]{figs/U-plots-Base (100M).pdf}
      \caption{The performance of relative learning rates, $\lambda_{\text{small}}$ extrapolated to larger models. We vary relative values for a given component while keeping all others fixed at their optimal values. The figure shows varying starting learning rates, the ablation for end rates can be found in Appendix. The optimal $\lambda_{\text{small}}$ (green) is compared to larger and smaller relative rates (white) and the baseline learning rate (red) at $\lambda=1$. The plot shows standard boxplots with 1.5 interquartile range.  Each boxplot is based on 10 experiments on \moe{113}.}
      %Values of all other relative variables are from the tuned configuration. 
      \label{fig:uplots_base}
    \end{figure*}
    ...
    \begin{figure}[H]  % The empty optional argument can be used for width adjustment
      \centering
      \includegraphics[width=\textwidth]{figs/U-plot-Attention.pdf} 
    ...
    \begin{figure}[h]
        ...
        % \hspace{1cm} %
        \begin{minipage}[t]{0.45\textwidth}
            \includegraphics[width=\linewidth]{figs/large_relative_vs_baseline.pdf}
    ...
    \begin{figure}[h]  % The empty optional argument can be used for width adjustment
      \centering
      \includegraphics[width=0.75\textwidth]{figs/relativity_motivation.pdf} 
