...
def make_relative_param_groups(args, model):
    ...
    for name, param in model.named_parameters():
        ...
        lr_fraction_dict[(relative_lr, relative_fraction)].append((param, name))

    # print(list(lr_fraction_dict.items())[0])

    param_groups = [
        {
            "params": [param for param, value in values],
            "names": [value for param, value in values],
            "lr": relative_lr * lr,
            "fraction": relative_fraction * final_lr_fraction,
        }
        for (relative_lr, relative_fraction), values in lr_fraction_dict.items()
    ]
...
def main(
    ...
    with open("param_groups_new.txt", "w") as f:
        f.write("------------printitn param groups--------------\n")
        for group in param_groups:
            f.write(f"name: {group['names']}\nstart: {round(group['lr'] / args.learning_rate, 4)}, end: {round(group['fraction'] / args.final_lr_fraction, 4)}\n")
