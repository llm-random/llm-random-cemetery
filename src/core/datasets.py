...
class AbstractDataset(IterableDataset):
    ...
    def sample_packer(self, sampler=None):
        buffer: List[int] = []
        if sampler is None:
            sampler_iter = iter(self.get_infinite_sampler())
        else:
            sampler_iter = iter(sampler)

        if self.use_new_sampling_method:
            while True:
                next_sampler_iter = next(sampler_iter)
                sample = next_sampler_iter["input_ids"]

                print(f"Rank {self.rank} processing document ID {next_sampler_iter['id']}")
            ...
            while True:
                tokens = next(sampler_iter)["input_ids"]
    ...
    def __iter__(self):
        ...
        # When world_size_independent is True, slice the underlying document iterator so each rank
        # consumes a disjoint subset of documents. This prevents multiple ranks from processing
        # the same documents while still allowing each rank to see the full dataset semantics.
        if self.world_size_independent:
            sampler = itertools.islice(
                self.get_infinite_sampler(), self.rank, None, self.world_size
            )
            return self.sample_packer(sampler=sampler)
        else:
