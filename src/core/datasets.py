...
def llama_tokenize_fn():
    ...
    tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.1-8B-Instruct", add_bos_token=True, add_eos_token=True, legacy=False)
    # tokenizer = LlamaTokenizerFast.from_pretrained("meta-llama/Llama-3.1-8B-Instruct", add_bos_token=True, add_eos_token=True, legacy=False)
    # tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.2-1B", add_bos_token=True, add_eos_token=True, legacy=False)
    # tokenizer = LlamaTokenizerFast.from_pretrained("meta-llama/Llama-3.2-1B", add_bos_token=True, add_eos_token=True, legacy=False)
    # tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.1-8B", add_bos_token=True, add_eos_token=True, legacy=False)
    # tokenizer = LlamaTokenizerFast.from_pretrained("meta-llama/Llama-3-7B", add_bos_token=True, add_eos_token=True, legacy=False)
    # tokenizer = LlamaTokenizerFast.from_pretrained("meta-llama/Llama-3.1-8B")
    # tokenizer = GPT2TokenizerFast.from_pretrained("gpt2")
    def tokenize_function(examples):
        batch_encodings = tokenizer(
            examples["text"],
...
class AbstractDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        # logger.info(f"self.rank {self.rank}, worker_id {worker_id}, num_workers {num_workers}, self.world_size {self.world_size}, worker_info {worker_info}, ")
