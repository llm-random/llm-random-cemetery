...
class GenericDataset(IterableDataset):
    ...
    def __init__(
        self,
        sequence_length: int,
        tokenize_fn: Callable, #TODO refactor so that tokenize_fn is specified in model's config
    ...
    def _load_hf_dataset(self, path, split):
        ...
        if self.shuffle:
            # Pre-shuffle the map-style dataset before sharding so each shard
            # gets a globally random draw of documents rather than a contiguous
            # sorted slice (fineweb-edu-dedup is stored in quality-score order).
...
class MixtureOfDatasets(IterableDataset):
    ...
    def __init__(
        ...
        self.datasets = [
            ...
            for path, eval_fraction in zip(paths, eval_fractions)
        ]
...
def _derive_dataset_name(path: str) -> str:
    """Infer a human-readable name from a dataset path.

    For paths ending in a split directory (train/val/validation/test) the
    parent directory name is used, e.g. '.../fineweb-edu-dedup/train' → 'fineweb-edu-dedup'.
    """
    path = path.rstrip("/")
    basename = os.path.basename(path)
    if basename in ("train", "val", "validation", "test"):
        return os.path.basename(os.path.dirname(path))
    return basename


def get_per_dataset_dataloaders(
    ...
    """Return a list of (name, eval_dataloader) for each dataset.

    Each dataloader covers only the eval slice of its own dataset (no mixture).
    The split follows the same eval_fraction logic as the mixture dataloader.
    """
    ...
    for d in datasets:
        path = d["path"]
        name = d.get("name", _derive_dataset_name(path))
        eval_fraction = d.get("eval_fraction", None)
