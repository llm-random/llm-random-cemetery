...
class TrainerDistillation(Trainer):
    ...
    Additional Args:
        ...
        distillation_alpha: Weight for distillation loss (0.0 to 1.0, default 0.5)
                           Ignored if use_dynamic_alpha is True
        distillation_temperature: Temperature for softening probability distributions (default 2.0)
        use_dynamic_alpha: If True, compute alpha dynamically as L_logits / L_is (default False)
    """
    ...
    def __attrs_post_init__(self):
        ...
        self.alpha_averaged_100 = AveMetric(100, "steps/100/train/alpha")
        
        if self.use_dynamic_alpha:
            logger.info(f"Distillation trainer initialized with DYNAMIC alpha (L_logits / L_is), "
                       f"temperature={self.distillation_temperature}")
        else:
            logger.info(f"Distillation trainer initialized with FIXED alpha={self.distillation_alpha}, "
                       f"temperature={self.distillation_temperature}")
    ...
    def calculate_loss(self, batch):
        """Override to compute both CE loss and distillation loss with dynamic alpha"""
        def _compute_losses(input_ids, target_ids):
            ...
            # Cross-entropy loss (L_CLM or L_is in the paper)
            ...
            # Distillation loss (L_logits in the paper - KL divergence)
            distill_loss = self.compute_distillation_loss(student_logits, teacher_logits)
            
            # Compute alpha dynamically or use fixed value
            if self.use_dynamic_alpha:
                # Dynamic alpha: α = L_logits / L_is
                # Clamp to [0, 1] to prevent extreme values
                alpha = torch.clamp(distill_loss / (ce_loss + 1e-8), 0.0, 1.0)
                alpha_value = alpha.item()
            else:
                alpha_value = self.distillation_alpha
            
            # Combined loss: L = L_CLM + L_logits + α × L_is
            # Rewritten as: L = (1 - α) × L_CLM + α × L_logits
            total_loss = (1.0 - alpha_value) * ce_loss + alpha_value * distill_loss
            ...
            return total_loss, ce_loss, distill_loss, alpha_value
        ...
        for batch_chunk in batch.chunk(self.gradient_accumulation_steps):
            ...
            total_loss, ce_loss, distill_loss, alpha = _compute_losses(input_ids, target_ids)
        ...
        avg_alpha = torch.tensor(alphas, device=self.device).mean()
        ...
        if dist.is_initialized():
            ...
            dist.all_reduce(avg_alpha, op=dist.ReduceOp.SUM)
            avg_alpha = avg_alpha / float(os.environ["WORLD_SIZE"])
        
        world_size = float(os.environ["WORLD_SIZE"])
        
        # Store individual losses and alpha for logging
        self._last_ce_loss = avg_ce_loss / world_size
        self._last_distill_loss = avg_distill_loss / world_size
        self._last_alpha = avg_alpha
        
        return avg_total_loss / world_size

    def log_metrics(self, loss, grad_norm):
        """Override to add distillation-specific metrics including alpha"""
        ...
        if hasattr(self, '_last_ce_loss'):
            ...
            # Alpha value (fixed or dynamic)
            self.metric_logger.log("steps/train/alpha", self.step, self._last_alpha.item())
            self.metric_logger.log("tokens/train/alpha", self.processed_tokens, self._last_alpha.item())
            ...
            self.alpha_averaged_100.log(self.metric_logger, self.step, self._last_alpha.item())
        ...
        with torch.no_grad():
            for _ in range(self.n_eval_steps):
                ...
                if hasattr(self, '_last_ce_loss'):
                    ...
                    alphas.append(self._last_alpha.item())
            ...
            if ce_losses:
                ...
                avg_alpha = torch.tensor(alphas).mean()
                ...
                self.metric_logger.log("steps/eval/alpha", self.step, avg_alpha.item())
                ...
                self.metric_logger.log("tokens/eval/alpha", self.processed_tokens, avg_alpha.item())
