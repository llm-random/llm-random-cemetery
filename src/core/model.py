...
def wrap_model_fsdp(model, fsdp_config):
    ...
    device_type = "cuda"  # str, not list
    devices = torch.arange(torch.cuda.device_count())  # tensor of global ranks
    mesh = DeviceMesh(device_type, devices)  # optional: add mesh_dim_names=("dp",)
