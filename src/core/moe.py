...
class TorchMoEFeedForward(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        expert_weights = torch.zeros_like(router_probs, dtype=hidden_states.dtype)
        expert_weights.scatter_(
            dim=-1,
            index=selected_experts,
            src=router_weights.to(hidden_states.dtype),
        )

        # Evaluate all experts in parallel and zero out the ones the router did not pick.
        ...
        if self.ff_pre_act.bias is not None:
            ff_pre_act = ff_pre_act + self.ff_pre_act.bias.unsqueeze(0)
            gate = gate + self.gate.bias.unsqueeze(0)
        ...
        if self.ff_post_act.bias is not None:
            expert_outputs = expert_outputs + self.ff_post_act.bias.unsqueeze(0)

        output = torch.einsum("te,teh->th", expert_weights, expert_outputs)
