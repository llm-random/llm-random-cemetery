...
def get_full_checkpoint_path(path):
    ...
    return (
        f"{path}/{slurm_array_task_id}" #dev TODO should be used slurm job id or slurm array job if + task id combination  
...
def load_checkpoint_from_file(load_config, model, optimizer, scheduler):
    ...
    # checkpoint_path = get_full_checkpoint_path( #dev TODO doesnt needed for loading, mixies independent loading job with saving job
    #     load_config.path
    # )
    # checkpoint_path = _find_latest_checkpoint(checkpoint_path) # TODO may be breaking scattered model loading

    print(f"will be loading model path: {checkpoint_path}") #dev
