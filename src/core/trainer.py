...
from projected_compression.compression import finalize_projection_weights
    ...
    def train(self):
        ...
        if self._should_save_final_checkpoint:
            if self.checkpoint.save.type == "nano":
                ...
                if os.environ["RANK"] == "0":
                    local_model = self.model.cpu()
                    finalize_projection_weights(local_model)
                    
                    self.print_state_dict_info(local_model.state_dict())
