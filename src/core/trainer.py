...
class Trainer:
    ...
    def train(self):
        for step, batch in zip(
            ...
            # Grad placement diagnostic
            if hasattr(self.model, 'encoder') and hasattr(self.model.encoder, 'blocks'):
                q = self.model.encoder.blocks[0].attention_layer.layer.q_proj
                if hasattr(q, 'projection_in_weight') and q.projection_in_weight.grad is not None:
                    ...
                    if os.environ.get("RANK", "0") == "0":
                        print(f"[OLD_PC_GRAD] local_shape={tuple(local_g.shape)} local_norm={local_g.float().norm():.4f} global_norm={local_sq.sqrt():.4f} placement={placement}")
