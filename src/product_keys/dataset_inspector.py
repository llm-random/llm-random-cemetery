...
def inspect_dataset(dataloader: DataLoader, num_samples=10):
    ...
    print(f"[512, 1024): {buckets[2] / num_samples * 100:.2f}%")
    print(f">1024: {buckets[3] / num_samples * 100:.2f}%")
...
def get_eurlex_dataset(seq_len: int, split: str = "test") -> DataLoader:
    dataset = GlueDataset(
        task_name="eurlex",
        sequence_length=seq_len,
        path=f"data/ft_dataset/eurlex/",
        split=split,
        seed=123,
        use_new_sampling_method=True,
        shuffle=True,
...
if __name__ == "__main__":
    ...
    dataset = get_eurlex_dataset(seq_len, split = "train")
