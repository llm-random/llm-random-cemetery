    ...
    for example in zip(*examples):
        if example[0] is not None:
            try:
                ...
                logger.error(f"However {example[0][:5]}")
                logger.error(f"{example[1][:5]}")
                logger.error(f"{example[2][:5]}")
                continue
        
    return [glue_collate_wrapper(example) if example is not None or example[0] is not None else None for example in zip(*examples)]
