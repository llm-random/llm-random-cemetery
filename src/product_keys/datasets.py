...
def glue_split_collate_wrapper(examples):
    for example in zip(*examples):
        # print("Ex 304:", example)
        if example[0] is not None:
            try:
                col_example = glue_collate_wrapper(example)
                
            except Exception as e:
                logger.error(f"Error collating example: {e}")
                logger.error(f"However Index 0: {example[0][:5]}")
                logger.error(f"Index 1: {example[1][:5]}")
                logger.error(f"Index 2: {example[2][:5]}")
                continue
        
    return [glue_collate_wrapper(example) if example is not None and example[0] is not None else None for example in zip(*examples)]
