...
class FinetuningTrainerMultiLabel(FinetuningTrainer):
    ...
    def calculate_loss(self, batch, is_dummy=False) -> LossResult:
        ...
        for texts_chunk, labels_chunk, attention_masks_chunk in zip(texts_chunks, labels_chunks, attention_masks_chunks):
            ...
            loss, chunk_correct, chunk_total, chunk_tp, chunk_fp, chunk_fn = _hack_for_python_garbage_collection(
                texts_chunk, labels_chunk, attention_masks_chunk
            )
    ...
    def full_eval(self):
        ...
        with torch.no_grad():
            ...
            if tp_count + fp_count + fn_count > 0:
                ...
                self.metric_logger.log("steps/eval/micro_f1", self.step, micro_f1)
                if not isinstance(self.metric_logger, (WandbLogger)):
                    self.metric_logger.log(
                        "tokens/eval/micro_f1", self.processed_tokens, micro_f1
                    )
    ...
    def full_eval_length_split(self):
        ...
        correct_counts = {split_value: 0 for split_value in self.split_values}
        total_counts = {split_value: 0 for split_value in self.split_values}
        tp_counts = {split_value: 0 for split_value in self.split_values}
        fp_counts = {split_value: 0 for split_value in self.split_values}
        fn_counts = {split_value: 0 for split_value in self.split_values}
        n_splits = len(self.split_values)
        ...
        def make_dummy_batch():
            ...
            bsz = self.eval_dataloader.batch_size
            dummy_text = torch.zeros((bsz, seq_len), dtype=torch.long)
            dummy_labels = torch.zeros((bsz, self.num_labels), dtype=torch.float32)
            dummy_mask = torch.zeros((bsz, seq_len), dtype=torch.bool)
            return (dummy_text, dummy_labels, dummy_mask)
        ...
        with torch.no_grad():
            ...
            for split_value, loss_list in losses.items():
                if len(loss_list) > 0:
                    ...
                    tp = tp_counts[split_value]
                    fp = fp_counts[split_value]
                    fn = fn_counts[split_value]
                    if tp + fp + fn > 0:
                        precision = tp / (tp + fp) if (tp + fp) > 0 else 0.0
                        recall = tp / (tp + fn) if (tp + fn) > 0 else 0.0
                        micro_f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else 0.0
                        self.metric_logger.log(f"steps/eval/micro_f1_({min_value}-{split_value})", self.step, micro_f1)
                        if not isinstance(self.metric_logger, (WandbLogger)):
                            self.metric_logger.log(
                                f"tokens/eval/micro_f1_({min_value}-{split_value})", self.processed_tokens, micro_f1
                            )
                min_value = split_value
