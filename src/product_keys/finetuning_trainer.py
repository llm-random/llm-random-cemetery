...
class FinetuningTrainerTwoSentences(FinetuningTrainer):
    ...
    def _forward_chunk(self, texts_chunk, labels_chunk, attention_masks_chunk, valid_in_chunk):
        ...
        # Register a one-shot forward hook on backbone.embedding.
        # FSDP wraps LLM into FSDPLLM, which proxies sub-module attribute
        # access (backbone.embedding works) but NOT custom method calls
        # (backbone.embed() would fail). The hook fires after the embedding
        # forward and adds segment embeddings to the token embeddings, while
        # leaving the full backbone(texts_chunk, ...) call intact — FSDP-safe.
