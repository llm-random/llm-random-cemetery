    ...
    def full_eval_length_split(self):
        ...
        with torch.no_grad():
            for _ in range(self.full_eval_batches):
                ...
                # 1. Check if ANY rank has valid data for each split
                has_data = torch.tensor([0.0 for _ in batch_list], device=self.device)
                ...
                for i, batch in enumerate(batch_list):
                    if has_data[i] == 0.0:
                        # ALL ranks have None for this split, safe to skip
                        continue

                    is_dummy = False
                    if batch is None:
                        # We are out of data, but another rank is NOT.
                        # Feed a dummy batch to keep FSDP synchronized!
                        is_dummy = True
                        seq_len = self.eval_dataset.dataset.sequence_length
                        bsz = self.gradient_accumulation_steps
                        dummy_text = torch.zeros((bsz, seq_len), dtype=torch.long)
                        dummy_labels = torch.zeros((bsz, seq_len), dtype=torch.long)
                        dummy_mask = torch.zeros((bsz, seq_len), dtype=torch.bool)
                        batch = (dummy_text, dummy_labels, dummy_mask)
