...
class CosineScheduler(_LRScheduler):
    ...
    def __init__(self, optimizer, n_steps, final_lr_fraction=0, last_epoch=-1):
        """
        Args:
    ...
    def get_lr(self):
        ...
        return [
            ...
            (1 + math.cos(math.pi * self.last_epoch / self.n_steps)) / 2
...
class RepeatedScheduler:
    ...
    def __init__(self, base_scheduler, num_cycles, **kwargs):
        """
        Args:
            ...
            **kwargs: Additional arguments (e.g., n_steps from main.py), passed to base_scheduler if it's a partial
        """
        self.num_cycles = num_cycles

        # If base_scheduler is a partial, we need to modify n_steps before instantiating
        if hasattr(base_scheduler, 'func'):
            # It's a partial, merge kwargs and modify n_steps
            if 'n_steps' in kwargs:
                kwargs['n_steps'] = kwargs['n_steps'] // num_cycles
            base_scheduler.keywords.update(kwargs)
