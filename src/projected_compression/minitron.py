...
def calculate_dimension_importances(model: nn.Module, calibration_data, dmodel, dff):
    ...
    dff_importance = torch.zeros(model.encoder.n_blocks, dff, device=device)
    ...
    with torch.no_grad():
        for batch in calibration_data:
            ...
            i = 0
            for layer in model.encoder.blocks:  #TODO dla każdego bloku osobno
...
def prune(model: nn.Module, dmodel_indices, dff_indices):
    ...
    for block in model.encoder.blocks:
        block_state_dict = block.state_dict() #TODO dla każdego bloku osobno
...
def minitron_prune(model: nn.Module, dataloader, dmodel, dff, calibration_dataset_size, seq_len, total_batch_size):


    # get 1024 samples calibration data

    # calibration_dataset_size = 64
    ...
    for i, batch in enumerate(dataloader):
        print(f"Calibration batch shape: {batch.shape}")
    ...
    dmodel_top_indices = torch.topk(importance_neurons, dim=0, largest=True, k=topk_dmodel).indices.tolist()

    dff_top_indices = []
    for i in range(model.encoder.n_blocks):
        dff_top_indices_current = torch.topk(importance_dmodel, dim=0, largest=True, k=topk_dff).indices.tolist()
        dff_top_indices.append(dff_top_indices_current)

    # dmodel_top_indices.sort()
    # dff_top_indices.sort()
