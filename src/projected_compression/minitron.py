...
def calculate_dimension_importances(model: nn.Module, calibration_data, dmodel, dff):
    ...
    dmodel_importance = torch.zeros(dmodel, device='cpu')
    # dff_importance = torch.zeros(dff, device=device)
    dff_importance = torch.zeros(dff, device='cpu')
...
def minitron_prune(model: nn.Module, dataloader, dmodel, dff, calibration_dataset_size, seq_len):
    ...
    calibration_data = torch.zeros(calibration_dataset_size // 4, 4, seq_len, dtype=torch.long, device='cpu')
    for i, batch in enumerate(dataloader):
        if i * 4 >= calibration_dataset_size:
            break
        calibration_data[i] = batch[:, :seq_len].to('cpu')
    ...
    dff_top_indices = torch.topk(importance_dmodel, dim=0, largest=True, k=topk_dff).indices.tolist()
    ...
    path = "./top_indices_" + str(rank) + ".json"
