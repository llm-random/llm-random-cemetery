...
def calculate_dimension_importances(model: nn.Module, calibration_data, dmodel, dff):
    ...
    for layer in model.encoder.blocks:
        ...
        normalized_pre_attn = layer.attention_layer.norm(x)
        dmodel_importance += torch.sum(normalized_pre_attn, dim=[0, 1])  # Sum across batch and sequence dimensions

        attention_output = layer.attention_layer.layer(normalized_pre_attn)
        x = x + attention_output  # Residual connection


        normalized_pre_ff = layer.ff_layer.norm(x)
        dmodel_importance += torch.sum(normalized_pre_ff, dim=[0, 1])  # Sum across batch and sequence dimensions
        ff_layer = layer.ff_layer.layer
        ff_pre_act = ff_layer.ff_pre_act(normalized_pre_ff)
        dff_importance += torch.sum(ff_pre_act, dim=[0, 1])

        ff_gated = ff_layer.silu(ff_layer.gate(normalized_pre_ff)) if hasattr(ff_layer, 'gate') else normalized_pre_ff
        ff_output = ff_layer.ff_post_act(ff_pre_act * ff_gated)
...
def prune(model: nn.Module, dmodel_indices, dff_indices):
    ...
    for block in model.encoder.blocks:
        ...
        if 'ff_layer.layer.gate.weight' in block_state_dict:
...
def minitron_prune(model: nn.Module, dataloader, dmodel, dff, calibration_dataset_size, seq_len):
    ...
    calibration_data = torch.zeros(calibration_dataset_size, seq_len + 1, dtype=torch.long, device=device)
    for i, batch in enumerate(dataloader):
        if i >= calibration_dataset_size:
            break
        calibration_data[i] = batch[0]
    ...
    # bottomk_dff = 5120
    #
    # dmodel_bottom_indices = torch.topk(importance_neurons, dim=0, largest=False, k=bottomk_dmodel).indices
    # dff_bottom_indices = torch.topk(importance_dmodel, dim=0, largest=False, k=bottomk_dff).indices
    #
    #
    # model = prune(model, dmodel_bottom_indices, dff_bottom_indices)
