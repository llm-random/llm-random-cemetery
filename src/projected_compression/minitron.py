...
def minitron_prune(model: nn.Module, dataloader, dmodel, dff, calibration_dataset_size, seq_len, total_batch_size, n_blocks):
    ...
    for i in range(n_blocks):
        print(f"Calculating top indices for block {i+1}/{n_blocks}, topk_dff={topk_dff}, dff_importance.shape={dff_importance.shape}")
        dff_top_indices_current = torch.topk(dff_importance, dim=0, largest=True, k=topk_dff).indices.tolist()
