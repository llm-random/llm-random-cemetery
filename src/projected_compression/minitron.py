...
def calculate_dimension_importances(model: nn.Module, calibration_data, dmodel, dff):
    ...
    with torch.no_grad():
        for batch in calibration_data:
            ...
            assert torch.allclose(x, model(document)), "Model output does not match expected output"
...
def minitron_prune(model: nn.Module, dataloader, dmodel, dff, calibration_dataset_size, seq_len):
    ...
    calibration_data = torch.zeros(calibration_dataset_size, 4, seq_len, dtype=torch.long, device=device)
    for i, batch in enumerate(dataloader):
        if i * batch.shape[0] >= calibration_dataset_size:
            break
