    ...
    def train(self):
        for step, batch in zip(
            ...
            if self.only_compress_model_gradient_clipping:
                ...
                proj_start_norm = torch.nn.utils.get_total_norm(
                    [v.grad for v in self.model.projections.parameters() if v.grad is not None]
                )
                total_grad_norm = torch.norm(
                    torch.stack([proj_start_norm.float()] + [n.float() for n in projection_grad_norms])
                ) if projection_grad_norms else proj_start_norm
            else:
