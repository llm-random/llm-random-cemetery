    ...
    def train(self):
        for step, batch in zip(
            ...
            if self.only_compress_model_gradient_clipping:
                ...
                grad_norm = total_grad_norm
                full_norm = torch.nn.utils.get_total_norm(
                    [p.grad for p in self.model.parameters() if p.grad is not None]
                )
                if int(os.environ.get("RANK", 0)) == 0:
                    print(f"[NORM_COMPARE] global_norm={grad_norm.item():.4f} full_norm={full_norm.item():.4f} ratio={full_norm.item()/grad_norm.item():.4f}", flush=True)
