...
class MemoryEfficientProjectedCompression(nn.Module):
    ...
    def _block_to_gpu(self, block_source, block_proj):
        """Move source + proj block to GPU for compute and optimizer step."""
        block_source.to('cuda')
        block_proj.to('cuda')

    def _block_to_cpu(self, block_source, block_proj):
        """Move source + proj block back to CPU after the optimizer step.
        Grads are already None (zero_grad called before this)."""
        block_source.to('cpu')
        block_proj.to('cpu')
        ...
        if optimizers is None:
            ...
            if shared_gradient_norms:
                ...
                for block_target, block_source, block_proj, optimizer, scheduler in zip(
                    ...
                    # Optimizer step on GPU: move state up, step, move state back,
                    # then return params to CPU.
                    if use_block_gpu:
                        self._move_block_optimizer_state(optimizer, 'cuda')
                    optimizer.step()
                    optimizer.zero_grad()
                    if use_block_gpu:
                        self._move_block_optimizer_state(optimizer, 'cpu')
                        self._block_to_cpu(block_source, block_proj)
                    torch.cuda.synchronize()
