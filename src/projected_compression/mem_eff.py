...
class MemoryEfficientProjectedCompression(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        if self.cpu_offload_projections:
            # Use the combined embedding table pre-computed on CPU and copied to GPU
            # in prepare_compressed_weights (analogous to how Wc is handled for encoder blocks).
            x = F.embedding(args[0], self._combined_embedding)
        else:
    ...
    def _copy_projected_weight(self, proj_comp, source_weight, target_weight):
        ...
        cpu_offload mode: rank 0 computes on CPU, broadcasts the full result via NCCL
        to all ranks, then distribute_tensor shards it into each rank's GPU DTensor.
        Normal mode: all ranks compute using DTensors on GPU."""
        if self.cpu_offload_projections:
            ...
            target_weight.copy_(proj_comp.get_projected_weight(self._get_source_weight(source_weight)))

    def _ensure_cpu_threads(self):
        """Set intra-op thread count once for rank 0 CPU matmuls.
        GPU training scripts commonly export OMP_NUM_THREADS=1 to prevent CPU/NCCL
        contention — which makes CPU matmuls single-threaded and ~80x slower.
        We override it here for rank 0, using (total_cpus / world_size) threads."""
        if hasattr(self, '_cpu_threads_configured'):
            return
        self._cpu_threads_configured = True
        if not dist.is_initialized() or dist.get_rank() == 0:
            # Ranks 1-3 are blocked in dist.broadcast while rank 0 computes — no
            # CPU contention, so rank 0 can use all cores.
            n_cpus = os.cpu_count() or 80
    ...
    def prepare_compressed_weights(self):
        ...
        with torch.no_grad():
            if self.cpu_offload_projections:
                # Pre-compute combined embedding table on CPU (rank 0 only), broadcast,
                # store as a GPU leaf tensor for the forward pass.
                # E_combined[t] = source_emb[t] @ P_emb.T + aux_emb[t]
                rank = dist.get_rank() if dist.is_initialized() else 0
                vocab, target_dmodel = self.projections.auxiliary_embedding_weights.weight.shape
                combined_gpu = torch.empty(vocab, target_dmodel, device='cuda', dtype=torch.float32)
                if rank == 0:
                    source_emb = self.source_model.embedding.weight.detach()  # [vocab, base_dmodel]
                    proj_emb = self.projections.embedding.detach()  # [target_dmodel, base_dmodel]
                    aux_emb_w = self.projections.auxiliary_embedding_weights.weight.detach()  # [vocab, target_dmodel]
                    combined_gpu.copy_(source_emb @ proj_emb.T + aux_emb_w)
                if dist.is_initialized():
                    dist.broadcast(combined_gpu, src=0)
                # requires_grad=True: autograd accumulates grad over micro-batches;
                # _backward_embedding_cpu() later translates it to CPU projection grads.
                self._combined_embedding = combined_gpu.requires_grad_(True)
    ...
    def pass_gradient_to_projections(
        ...
        if self.cpu_offload_projections:
            # Translate combined-embedding grad (GPU) → CPU grads for P_emb and aux_emb.
            self._backward_embedding_cpu()
    ...
    def backward_compressed_weights(self, proj, source_weight, target_weight):
        if self.cpu_offload_projections and hasattr(target_weight.grad, 'to_local'):
            ...
            # Normal path: all ranks compute using DTensors on GPU.
