    ...
    def pass_gradient_to_projections(
        ...
        if optimizers is None:
            ...
            if shared_gradient_norms:
                ...
                blocks_only_norm = torch.tensor([n.item() for n in projection_blocks_grad_norms]).norm()
                global_norm = torch.tensor(
                    [start_grad_norm.item()] + [n.item() for n in projection_blocks_grad_norms]
                ).norm()
                rank = int(os.environ.get("RANK", 0))
                print(f"[GRAD_NORM_DEBUG rank={rank}] head+emb={start_grad_norm.item():.4f} blocks={blocks_only_norm.item():.4f} total={global_norm.item():.4f}", flush=True)
                ...
                if int(os.environ.get("RANK", 0)) == 0:
                    print(f"[GRAD_NORM_DEBUG] head+emb={start_grad_norm.item():.4f} blocks={blocks_only_norm.item():.4f} total={final_grad_norm.item():.4f}", flush=True)
