    ...
    def pass_gradient_to_projections(
        ...
        if optimizers is None:
            for i, (block_target, block_source, block_proj) in enumerate(zip(
                ...
                # Grad placement diagnostic for block 0 only
                if i == 0:
                    import os as _os
                    q = block_proj.compressible_q
                    if hasattr(q, 'projection_in_weight') and q.projection_in_weight.grad is not None:
                        g = q.projection_in_weight.grad
                        local_g = g.to_local() if hasattr(g, 'to_local') else g
                        placement = str(g.placements) if hasattr(g, 'placements') else 'plain_tensor'
                        local_sq = local_g.float().norm() ** 2
                        import torch.distributed as _dist
                        if _dist.is_initialized():
                            _dist.all_reduce(local_sq, op=_dist.ReduceOp.SUM)
                        if _os.environ.get("RANK", "0") == "0":
                            print(f"[MEMEFF_GRAD] local_shape={tuple(local_g.shape)} local_norm={local_g.float().norm():.4f} global_norm={local_sq.sqrt():.4f} placement={placement}")
            ...
            if shared_gradient_norms:
                ...
                target_norm_layer_norm = torch.nn.utils.get_total_norm(norm_layer_grads)
