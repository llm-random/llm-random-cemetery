...
class MemoryEfficientProjectedCompression(nn.Module):
    ...
    def _copy_projected_weight(self, proj_comp, source_weight, target_weight):
        ...
        if self.cpu_offload_projections and source_weight.device.type == 'cpu':
            if self.block_gpu_compute:
                # Transiently move proj params + source weight to GPU.
                # All ranks compute the same result independently — no broadcast.
                proj_comp.to('cuda')
                src_w_gpu = self._get_source_weight(source_weight).to('cuda')
                result_gpu = proj_comp.get_projected_weight(src_w_gpu)
                proj_comp.to('cpu')
    ...
    def backward_compressed_weights(self, proj, source_weight, target_weight):
        if (self.cpu_offload_projections
            ...
            if self.block_gpu_compute:
                # All ranks compute backward independently on GPU.
                # source + proj weights are replicated CPU tensors → same result on every rank.
                # Grads moved back to CPU for the block optimizer.
                proj.to('cuda')
                src_w = source_weight.detach().to('cuda')
                if self.cast_bfloat16:
                    src_w = src_w.bfloat16()
                weights = proj.get_projected_weight(src_w)
                weights.backward(wc_grad_gpu)
                # PyTorch requires grad and param on same device. Save CPU grads,
                # clear CUDA grads while params are still on CUDA, move to CPU, restore.
                cpu_grads = [(p, p.grad.detach().cpu()) for p in proj.parameters() if p.grad is not None]
                for p, _ in cpu_grads:
                    p.grad = None
                proj.to('cpu')
                for p, g in cpu_grads:
                    p.grad = g
            else:
                # CPU path: rank 0 computes, broadcasts grads to all ranks.
                rank = dist.get_rank() if dist.is_initialized() else 0
                if rank == 0:
