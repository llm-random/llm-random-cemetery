    ...
    def pass_gradient_to_projections(
        ...
        if optimizers is None:
            ...
            if shared_gradient_norms:
                ...
                # Debug: print detailed norms at step 1 to diagnose norm differences
                import os as _os
                if _os.environ.get("RANK", "0") == "0":
                    _head_norm = get_module_grad_norm(self.projections.head)
                    _emb_norm = torch.nn.utils.get_total_norm([self.projections.embedding.grad] if self.projections.embedding.grad is not None else [])
                    _aux_emb_norm = get_module_grad_norm(self.projections.auxiliary_embedding_weights)
                    _block_total = torch.tensor([n.item() for n in projection_blocks_grad_norms]).norm()
                    print(f"[NORM_DEBUG] start_grad_norm={start_grad_norm.item():.4f} head_proj_norm={_head_norm.item():.4f} emb_proj_norm={_emb_norm.item():.4f} aux_emb_norm={_aux_emb_norm.item():.4f} blocks_combined={_block_total.item():.4f}")
                    print(f"[NORM_DEBUG] head.projection_in_weight.grad type={type(self.projections.head.projection_in_weight.grad)}, is DTensor={hasattr(self.projections.head.projection_in_weight.grad, 'to_local')}")
