...
class MemoryEfficientProjectedCompression(nn.Module):
    ...
    def _block_to_gpu(self, block_source, block_proj):
        """Move one encoder block's source weights + projection weights to GPU."""
        block_source.to('cuda')
        block_proj.to('cuda')

    def _block_to_cpu(self, block_source, block_proj):
        """Move block source weights back to CPU; save proj grads as CPU tensors,
        then move proj back to CPU and restore the CPU grads.
        Called after GPU backward so the block optimizer can step on CPU."""
        block_source.to('cpu')
        # Grads must be on the same device as the param.  Save CPU copies, clear
        # CUDA grads while params are still on CUDA, move params to CPU, restore.
        cpu_grads = [(p, p.grad.detach().cpu()) for p in block_proj.parameters() if p.grad is not None]
            ...
            if shared_gradient_norms:
                ...
                if not dist.is_initialized() or dist.get_rank() == 0:
                    ...
                    _logger.warning(
                        ...
                        f"bwd={t_bwd_total:.3f}s  d2h={t_d2h_total:.3f}s  opt={t_opt_total:.3f}s  "
                        f"total={t_nccl_total+t_h2d_total+t_bwd_total+t_d2h_total+t_opt_total:.3f}s"
                    )
