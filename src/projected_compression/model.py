...
class ProjectedEmbedding(nn.Module):
    ...
    def finalize(self):
        device = "cpu"
        pweight = self.projection.full_tensor().float().to(device) @ self.embedding.weight.full_tensor().float().to(device) + self.auxiliary_weight.weight.full_tensor().float().to(device)
