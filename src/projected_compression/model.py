...
class ProjectedEmbedding(nn.Module):
    ...
    def finalize(self):
        device = "cpu"
        pweight = self.embedding.weight.full_tensor().float().to(device) @ self.projection.full_tensor().float().to(device)
        self.embedding.weight = None
        self.projection = None

        if os.environ["RANK"] == "0":
            self.finalized_weight = pweight
        else:
            self.finalized_weight = None
