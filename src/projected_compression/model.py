...
class ProjectedEmbedding(nn.Module):
    ...
    def finalize(self):
        device = "cpu"
        pweight = self.embedding.weight.full_tensor().float().to(device) @ self.projection.full_tensor().float().to(device) + self.auxiliary_weight.weight.full_tensor().float().to(device)
