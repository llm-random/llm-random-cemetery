...
def create_model(cfg_model, cfg_projected_compression, source_model_for_distillation):
    ...
    # Initializing model.projections — always on GPU (plain tensors, non-FSDP2).
    # Only source encoder blocks are kept in CPU RAM and moved per-block for compute.
    # Proj blocks + optimizer state stay on GPU permanently: avoids repeated PCIe
    # round-trips for proj params and optimizer state (m, v) per block per step.
