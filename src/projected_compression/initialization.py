...
def create_model(cfg_model, cfg_projected_compression, source_model_for_distillation):
    ...
    if not cpu_offload_projections and torch.distributed.is_initialized():
        ...
        cpu_sd = _build_projections_cpu_init_sd(meta_sd, dmodel_topk_indices, dff_topk_indices)
        sharded_sd = get_sharded_sd(meta_sd, cpu_sd)
        model.projections.load_state_dict(sharded_sd, assign=True)
...
def _build_projections_cpu_init_sd(meta_sd, dmodel_topk_indices, dff_topk_indices):
    """Build CPU init state dict for the full Projections module while params are meta DTensors.
    ...
    Uses the param name (blocks.{i}.*) and result dimension to select dmodel vs per-block
    dff indices.  Works for all params in Projections: embedding, head, and all blocks.
    """
