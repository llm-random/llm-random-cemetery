...
def create_model(cfg_model, cfg_projected_compression, source_model_for_distillation):
    ...
    # Fix: save block DTensor metadata (device_mesh + placements) before to_empty destroys it,
    # then re-shard after init so each GPU holds only 1/world_size of each block's params.
    ...
    if not cpu_offload_projections and torch.distributed.is_initialized():
        for block, meta_sd in zip(model.projections.blocks, block_meta_sds):
            block_sd = block.state_dict()          # plain CUDA, full size on each rank
            sharded_sd = get_sharded_sd(meta_sd, block_sd)
            block.load_state_dict(sharded_sd, assign=True)
        logger.info("Re-sharded proj blocks as FSDP2 DTensors.")
