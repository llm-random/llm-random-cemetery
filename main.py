...
def get_model_optimizer_scheduler(cfg, model, learning_rate):
    ...
    if (
        ...
        logger.info(
            f"Using regex-based optimizer parameter grouping with {len(cfg.trainer.optimizer_param_groups)} groups."
        )
        logger.info(
            f"Model parameters (and trainable status): {[f'{name} (trainable={param.requires_grad})' for name, param in model.named_parameters()]}"
        )
        ...
        for group_cfg in cfg.trainer.optimizer_param_groups:
            ...
            if "indices_filter" in group_cfg and group_cfg.indices_filter:
                ...
                if OmegaConf.is_list(raw_indices) or isinstance(raw_indices, (list, tuple)):
            ...
            for name, param in model.named_parameters():
                ...
                if re.search(group_regex, name):
                    ...
                    if target_indices is not None and layer_path_prefix:
                        ...
                        match_idx = re.search(f"{prefix_esc}\.(\d+)\.", name)
                        
                        if not match_idx:
                            # Does not match the path structure -> skip adding to this group
                            continue
                        
                        layer_idx = int(match_idx.group(1))
                        if layer_idx not in target_indices:
                            # Index matches structure but not the allowed values -> skip
                            continue
