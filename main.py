...
def run(cfg, metric_logger=None):
    ...
    if isinstance(metric_logger, NeptuneLogger) and (training_state["run_id"] is None and not cfg.infrastructure.metric_logger.new_neptune_job):
        print("really printing config???????????????????????????????????") #dev
