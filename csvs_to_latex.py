...
def load_csv(path: str, metric: str) -> tuple[str, dict[tuple[str, str], float]]:
    ...
    for row in rows:
        ...
        if key in cells and created <= freshest.get(key, ""):
            logger.warning("%s: %s is an older rerun of %s, skipping", path, row["model"], key)
            continue
        if key in cells:
            logger.warning("%s: %s supersedes an earlier row for %s", path, row["model"], key)
...
def render(
    ...
    lines.append(f"& \\multicolumn{{{n_ratios * n_groups}}}{{c}}{{\\textbf{{Model}}}} \\\\")
...
def main():
    ...
    parser.add_argument("csvs", nargs="+", help="CSVs from scrape_eval_results.py, one per model")
    parser.add_argument(
        "--labels",
        nargs="+",
        default=None,
    ...
    parser.add_argument("--out", default=None, help="write the .tex here instead of stdout")
